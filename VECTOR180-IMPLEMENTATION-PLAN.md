# Vector180 Implementation Plan

**Status:** active delivery roadmap; first-class standalone diagrams, shared
browser conformance, the writable trusted editor MVP, C5 2.0 typed
native-object patches plus one exact reviewed connector clone, C9 explicit atom
composition/baselines, C10 2.0 proof-carrying reconciliation, and C11 1.2
...
pre-production naming/lineage/diff gates are implemented, C7 through C11 retain
...
destination-neutral naming/lineage/diff cutover → banked text-resilience contracts →
...
Vector180 is already structurally well suited to live editing:

- exact source bytes and hashes remain authoritative;
- standalone `.vector180.svg` is a first-class semantic/editable atom rather than a
...
- self-contained `.vector180.html` is the explicit aggregation, theme, viewer, and
...
The current kernel now implements the narrow geometry/style/text claims defined
by the in-progress C6 2.0 successor. It resolves fixed-canvas deck primitives or an
...
deck slide can be hydrated into an independent `.vector180.svg` by dereferencing its
resolved styles and reloading it through the diagram path. C5 2.0 supports
...
presentation-attribute style while preserving direct text/deck-control 0.1
transactions. Its
`vector180-patch/0.1` envelope adds exactly one structural exception:
...
reinspection. Duplicate identity refuses by default. C10 2.0 accepts a strict
...
### 1.1 Completed pre-production product gates

Vector180 is now the destination-neutral canonical name across source suffixes,
wire vocabulary, package, CLI, skill, schemas, examples, and TypeScript API.
Pure PPTV 0.1 remains a frozen readable legacy dialect with explicit atom
migration and legacy-deck slide-hydration boundaries.

C4/C6 now define inert hydration/template/style-family metadata plus a derived
non-persistent palette fingerprint. C12 and the `diff` command provide
stable-ID semantic source comparison with lexical-only changes kept separate.
...
Children retain explicit local geometry and DOM order. C5 2.0 can replace an
...
calibration, Vector180/PPTX font embedding, and arbitrary web fonts remain deferred.
...
- a deterministic `*.editable.html` wrapper contains exact clean deck or
...
- editor save/export always produces exact clean canonical `.vector180.html` or
  `.vector180.svg`, never DOM serialization and never the wrapper;
...
- The exact-source editor session and deterministic strict-CSP pack now provide
  ...
  direct-text/theme/order only even though C5 2.0 now exposes typed
...
- C8's pure anchor-aware deck/diagram preflight, strict selected font map with
  one bundled exact ABeeZee default, exact-byte Fontkit adapter, worked-deck
  regression, environment-labeled browser adapter, and conservative editor
  overlays are implemented. Native PowerPoint calibration keeps that contract
  `in-progress`.
- C5 2.0 defines one `vector180-patch/0.1` envelope for direct text/deck
  operations plus rect/ellipse geometry, line endpoints,
  ...
  revalidation. The same envelope permits at most one
...
- C9's bounded atom path explicitly composes identity or aspect-preserving
  ...
  refused.
- C10 2.0 authenticates exact atom, composed-deck, map, placement, and PPTX
...
- C11 1.2 implements deterministic trusted standalone-SVG browser capture,
...
- Vector180 0.1.1 paragraph intent, reliable/editable PPTX export, and conservative
  baseline-free import grace are banked design only. Current source/container
  identifiers, schemas, and examples remain `0.1`; the current contract
  revisions are C4-C10 `2.0`, C11 `1.2`, and C12 `1.0`. C7-C11 retain their
  declared in-progress gates, and the npm package is
  `@office180/vector180@0.1.0-alpha.5`.
...
- deck-only hydrated single-slide `.vector180.svg` download;
...
C7 deliberately does not emit `.vector180.map.json` yet. Its native names,
...
`set-attribute` escape hatch. C5 2.0 retains the typed operations:
...
- one `clone-connector`, which exact-clones an existing childless native
...
CSS/inline/inherited styles are not rewritten. Direct text/deck-control
`vector180-patch/0.1` transactions remain compatible. Browser
drag/resize/style controls have not
...
`@office180/vector180`; core, ops, and browser code remain free of OpenDocKit,
...
OpenDocKit supplies fast structural regression coverage, not canonical Vector180
semantics.
...
within-parent sibling order into old-value-preconditioned `vector180-patch/0.1`
...
a separate `vector180 patch` action.
...
`vector180-reconcile-resolution/0.1` only when exactly one occurrence remains
...
style. That path emits `vector180-patch/0.1` with exactly one `clone-connector`.
...
exact embedded deck/diagram bytes + expected hash
  ...
  -> EditorSession
       ...
       -> C5 2.0 validate/apply (`vector180-patch/0.1`)
...
The first implementation remains inside `@office180/vector180`:
...
The portable session/kernel can apply the contracted C5 2.0 vocabulary, but
...
3. custom package properties for Vector180 source hash, compiler, resolved schema,
   active theme, and, for C9, originating atom; and
4. the C9 `.vector180.map.json`, which records the exact atom/composed-deck/PPTX
...
exact atom + C9 map + edited descendant PPTX
  ...
  -> propose typed C5 2.0 operations
...
3. expose browser-editor gestures for the safe C5 2.0 geometry, connector,
...
6. extend the proven exact-path native no-op bridge to representative,
