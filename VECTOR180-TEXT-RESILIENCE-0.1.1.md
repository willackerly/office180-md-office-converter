# Vector180 0.1.1 Text Resilience

**Status:** banked design; not implemented or accepted by current loaders

**Decision date:** 2026-07-30

**Version line:** originally banked as PPTV source/profile `0.1.1`, now carried
forward unchanged as planned Vector180 `0.1.1`; independent from npm, contract,
viewer-runtime, and agent-profile versions

## 1. Why bank this

Vector180's most important text-fidelity property is deterministic line membership:
...
This document records a future design target. It was banked while historical
`@office180/pptv@0.1.0-alpha.3` was current; the later PPTV alpha.4 round-trip
release and the Vector180 alpha.5 namespace migration deliberately leave the
same text semantics untouched. It does not change:

- the currently accepted source/container identifier `0.1`;
- current `@office180/vector180@0.1.0-alpha.5` runtime behavior;
- C4–C10 at their Vector180 2.0 revisions or C11 1.2;
- the `vector180-resolved*/0.1`, `vector180-text-fit*/0.1`, browser, or compiler
...
<text
  ...
  data-vector180-text-intent="paragraph"
  data-vector180-frame="120 180 900 240"
  data-vector180-line-step="48">
...
- retain the authored Vector180 frame unchanged.
...
and remain explicit Vector180 lines. A baseline-aware reconciliation path uses the
...
4. Serialize the result immediately as explicit Vector180 lines with paragraph
   intent.
...
`0.1.1` in this document names the banked Vector180 source/profile capability line.
It is not:

- the current npm package version;
- contract revision `1.1`;
- viewer `vector180-browser/0.1`;
- compiler `office180-vector180-pptx-canary/0.1`; or
- agent profile `vector180-agent/1`.
