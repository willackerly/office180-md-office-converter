...
multiline control, but the model retains separate hard lines. C8 can report
horizontal overflow after a longer edit, but cannot wrap, shrink, or move a
word.
...
### Checked browser conformance evidence

Browser conformance reuses the portable C4/C6 TypeScript implementation; it
does not delegate meaning to the DOM, CSSOM, or a browser layout engine.
`src/browser/runtime.ts` emits one immutable JSON-safe normalization for direct
Node/browser equality checks.

The checked browser artifact is
`packages/pptv/assets/pptv-browser-kernel-0.1.iife.js`, generated by exact
`esbuild@0.28.1` as an ES2022 IIFE. Its registered size is 700,262 bytes and
its SHA-256 is
...
Node built-in, Fontkit, or JSZip dependency.

Real-HTTP conformance under exact `@playwright/test@1.62.0` compares complete
normalized C4/C6 results for the minimal HTML deck, the standalone kitchen-sink
diagram with arbitrary `viewBox="-20 10 960 640"`, and a deliberately invalid
standalone profile. The current seven-test-per-project browser/editor matrix
passed in Chromium 151.0.7922.34, Firefox 153.0, and WebKit 26.5 on 2026-08-01:
21 of 21 project/test results. The C4/C6 parity corpus itself is three fixtures
per engine.
This closes semantic Node/browser parity for the declared corpus; it does not
claim pixel-render parity, browser text-metric parity, or downstream native
PowerPoint fidelity.
...
- It adds or normalizes the standalone root version and SVG namespace,
  prepends the canonical non-normative `pptv-authoring` discovery comment, and
  leaves the authored viewBox unchanged. The comment is writer metadata rather
  than a C4 validity requirement or semantic authority.
...
- Verification/build-only: exact `esbuild@0.28.1` and
  `@playwright/test@1.62.0`; neither participates in resolution semantics
...
- `packages/pptv/src/browser/runtime.ts` — shared C4/C6 normalized browser
  conformance projection
- `packages/pptv/scripts/build-browser-kernel.mjs` — exact deterministic IIFE
  build/check and portable-input enforcement
- `packages/pptv/assets/pptv-browser-kernel-0.1.iife.js` and
  `pptv-browser-kernel-0.1.meta.json` — checked browser artifact and build
  identity
- `packages/pptv/e2e/browser-conformance.spec.ts` — three-engine parity corpus
...
- `packages/pptv/test-fixtures/c6/` — valid kitchen-sink and invalid-profile
  standalone fixtures
