...
**Source:** `ROADMAP.md` §6, `PPTV-IMPLEMENTATION-PLAN.md` §6,
...
C11 provides one renderer-neutral evidence envelope for Markdown/DOCX and
PPTV/PPTX round trips. It also defines a bounded local bridge for one native
...
- **C9/C10 PPTV workflows** — need source, generated, native-saved, edited,
  reconciled, and regenerated slide evidence.
...
PPTV SVG with pinned Playwright Chromium. It records source/image hashes,
...
validated standalone `*.pptv.svg` atom. A reconciled C10 result becomes
eligible only after its patch is applied to a new atom and that exact output
independently validates.

The adapter:

- requires explicit `--trusted`, repository-contained input/output, a
  `.pptv.svg` input, and new `.png` destination;
- runs `pnpm pptv validate` without executing source runtime content and
  re-hashes the same bytes before serving;
...
- Depends on lane-specific C3 or C9/C10 package and semantic validation; C11
...
- **Source docs:** `ROADMAP.md` §6, `PPTV-IMPLEMENTATION-PLAN.md` §6, and
...
- **Predecessor:** C11 1.0 remains in Git history; this 1.1 file is the sole
  current C11 contract.
- **Migration boundary:** existing `office180-visual-evidence/0.1` manifests
  remain valid; native automation opts into the separate bridge report and
  binder.
