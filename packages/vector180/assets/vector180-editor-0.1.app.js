/*! @office180/vector180 writable editor 0.1 | generated by esbuild 0.28.1 */
"use strict";
(() => {
...
`;
  ...
  async function materializeSource(input, options = {}) {
    ...
    const document2 = await createSourceDocument(input.name, text, bytes);
  ...
  async function scanVector180Source(input, options = {}) {
    ...
    const document2 = detection.dialect === void 0 ? materialized.document : sourceWithWireFamily(
      materialized.document,
      detection.dialect.wireFamily
    );
    const diagnostics = [...materialized.diagnostics];
    const mapper = new SourceMapper(document2.text, document2.bytes);
    if (detection.mixed) {
      diagnostics.push({
        code: "VECTOR180-NAMESPACE-MIXED",
        severity: "fatal",
    ...
    if (kind === "svg") {
      if (!validateStandaloneSvgXml(document2, mapper, diagnostics)) {
        return {
          kind,
          encoding: "utf-8",
          source: document2,
      ...
      return scanSvg(document2, mapper, diagnostics, options, detection.dialect);
    }
    return {
      kind,
      ...
      sections: [
        {
          kind: "manifest",
          range: mapper.range(0, document2.text.length),
          contentRange: mapper.range(
            document2.text.startsWith("\uFEFF") ? 1 : 0,
            document2.text.length
          ),
          attributes: {}
        }
      ],
  ...
  function validateThemeParity(deck, base, themes, diagnostics) {
    const required2 = [...base.tokenUses.keys()].sort();
    const requiredSet = new Set(required2);
    for (const theme of themes.values()) {
      const missing = required2.filter((token) => !theme.tokens.has(token));
  ...
  function resolveNodes(document2, scopeId, nodes2, rules, activeTheme, styles, diagnostics) {
    for (const node of nodes2) {
      ...
      if (styles.has(node.id)) {
        diagnostics.push({
          ...
          ...diagnosticScope(document2, scopeId),
          objectId: node.id
        });
        continue;
      }
  ...
  function resolveNodeStyle(document2, scopeId, node, rules, activeTheme, diagnostics) {
    const candidates = defaultCandidates();
    if (document2.sourceKind === "svg") {
      const styleAttribute = wireAttribute(document2.wireFamily, "style");
      const themeAttribute = wireAttribute(document2.wireFamily, "theme");
      const tokenPrefix = wireCssTokenPrefix(document2.wireFamily);
    ...
    for (const property of CSS_PROPERTIES) {
      ...
      const sourceRange = attributeRange(document2, node, property);
      ...
      for (const declaration of rule.declarations) {
        ...
        candidates.set(declaration.property, {
          ...
          provenance: {
            ...
            ...document2.sourceKind !== "html" || document2.baseStyle === void 0 ? {} : { sourceRange: document2.baseStyle.contentRange }
          }
        });
      }
    }
    ...
    if (inline !== void 0) {
      const range = attributeRange(document2, node, "style") ?? node.openTagRange;
  ...
  function attributeRange(document2, node, name) {
    const ranges = document2.index.objects.get(node.id)?.attributeRanges;
  ...
  function objectDiagnostic(code, message, document2, scopeId, node, attribute) {
    return {
      ...makeDiagnostic(
        code,
        message,
        attribute === void 0 ? node.sourceRange : attributeRange(document2, node, attribute) ?? node.openTagRange
      ),
      ...diagnosticScope(document2, scopeId),
      objectId: node.id
    };
  }
  function diagnosticScope(document2, scopeId) {
    return document2.sourceKind === "html" ? { slideId: scopeId } : { atomId: scopeId };
  }
  ...
  function indexSourceNodes(document2) {
    const nodes2 = /* @__PURE__ */ new Map();
    if (document2.sourceKind === "html") {
      for (const slide of document2.slides.values()) {
        visitSourceNodes(slide.children, (node) => nodes2.set(node.id, node));
      }
    } else {
      visitSourceNodes(document2.children, (node) => nodes2.set(node.id, node));
    }
    return nodes2;
  }
  ...
  function visitSourceNodes(nodes2, visitor) {
    for (const node of nodes2) {
      visitor(node);
      visitSourceNodes(node.children, visitor);
    }
  }
  function resolveSiblings(nodes2, scope, parentId, parentWorldOffset, context) {
    const result = [];
    for (const node of nodes2) {
      if (node.exportMode === "ignore") continue;
  ...
  function parseSourceElement(node, document2) {
    const source = document2.source.text.slice(
  ...
  function addHydrationMetadata(source, provenance) {
    ...
    const payload2 = canonicalJsonText({ hydration: provenance });
    const metadata = `
  <metadata data-vector180-metadata="vector180-atom-metadata/0.1">${payload2}</metadata>`;
  ...
  async function applyPatch(document2, input) {
    const trusted = await reloadPatchBase(document2);
    if (trusted.document === void 0) {
      return {
        applied: false,
        originalSha256: document2.source.sha256,
        affectedIds: [],
        edits: [],
  ...
  function planPatch(document2, input) {
    ...
    if (document2.wireFamily !== "vector180") {
      diagnostics.push({
    ...
    if (document2.sourceKind === "html" && !document2.materialization.complete) {
      diagnostics.push({
    ...
    if (patch.baseSha256 !== document2.source.sha256) {
      diagnostics.push({
    ...
    const deletingIds = collectDeletionIds(document2, patch.ops);
  ...
  async function reloadPatchBase(document2) {
    try {
      const reloaded = await reloadCandidate(document2, document2.source.text);
      if (reloaded.source.sha256 !== document2.source.sha256) {
        return {
          diagnostics: [
            {
              code: "VECTOR180-PATCH-INVALID-BASE",
  ...
  function planSetText(document2, operation, operationIndex, edits, affectedIds, diagnostics) {
  ...
  function resolvePatchState(document2) {
    const result = document2.sourceKind === "html" ? resolveVector180Deck(document2) : resolveVector180Atom(document2);
  ...
  function findObject(document2, id) {
    if (document2.sourceKind === "svg") {
      return findNode(document2.children, id);
    }
    for (const slideId2 of document2.slideOrder) {
      const slide = document2.slides.get(slideId2);
      if (slide === void 0) continue;
  ...
  function failedResult(document2, plan, message, related) {
    return {
      applied: false,
      originalSha256: document2.source.sha256,
      affectedIds: [],
  ...
  var EditorSession = class _EditorSession {
    ...
    constructor(document2, options) {
      this.originalSha256 = document2.source.sha256;
      this.history = [snapshot(document2)];
      this.historyLimit = normalizeHistoryLimit(options.historyLimit);
      this.author = options.author;
      this.integrityDiagnostics = options.expectedSha256 === void 0 || options.expectedSha256 === document2.source.sha256 ? [] : [
        {
          code: "VECTOR180-EDITOR-INTEGRITY",
          severity: "error",
          message: `Embedded source hash ${document2.source.sha256} does not match expected hash ${options.expectedSha256}.`
        }
      ];
    }
  ...
  function intentToOperation(document2, intent) {
    if (intent.kind === "set-active-theme") {
      if (document2.sourceKind !== "html" || document2.activeTheme === void 0) {
        return {
          diagnostics: [
            {
      ...
      return {
        operation: {
          op: "set-active-theme",
          theme: intent.theme,
          oldTheme: document2.activeTheme
        },
        diagnostics: []
      };
    }
    if (intent.kind === "set-slide-order") {
      ...
      return {
        operation: {
          op: "set-slide-order",
          order: [...intent.order],
          oldOrder: [...document2.slideOrder]
        },
        diagnostics: []
      };
    }
    const node = findObject2(document2, intent.id);
    if (node === void 0) {
      return {
        diagnostics: [
          {
  ...
  function snapshot(document2) {
    const common = {
      sourceText: document2.source.text,
      sourceSha256: document2.source.sha256
    };
    return document2.sourceKind === "html" ? Object.freeze({
      ...common,
      sourceKind: "html",
      document: document2,
      deck: document2
  ...
  function findObject2(document2, id) {
    if (document2.sourceKind === "svg") {
      return findNode2(document2.children, id);
    }
    for (const slide of document2.slides.values()) {
  ...
  function findNode2(nodes2, id) {
    for (const node of nodes2) {
      if (node.id === id) return node;
      const child = findNode2(node.children, id);
      if (child !== void 0) return child;
    }
    return void 0;
  }
  function hasStableTarget(document2, id) {
    if (document2.sourceKind === "svg") {
      return document2.id === id || document2.index.objects.has(id);
    }
    return document2.slides.has(id) || document2.themes.has(id) || document2.libraries.has(id) || document2.index.objects.has(id);
  }
  ...
  var SVG_NAMESPACE3 = "http://www.w3.org/2000/svg";
  var FIT_SEVERITY = {
    clear: 0,
    "near-limit": 1,
    overflow: 2,
  ...
  var nodes = {
    payload: required("#vector180-editor-payload"),
    title: required("[data-title]"),
    hash: required("[data-hash]"),
    integrity: required("[data-integrity]"),
    operationStatus: required("[data-operation-status]"),
    navigationTitle: required("[data-navigation-title]"),
    navigation: required("[data-navigation]"),
    deckControls: required("[data-deck-controls]"),
    theme: required("[data-theme]"),
    themeApply: required("[data-theme-apply]"),
    slideUp: required("[data-slide-up]"),
    slideDown: required("[data-slide-down]"),
    extract: required("[data-extract]"),
    viewport: required("[data-viewport]"),
    source: required("[data-source]"),
    tree: required("[data-tree]"),
    textEdit: required("[data-text-edit]"),
    textApply: required("[data-text-apply]"),
    textHelp: required("[data-text-help]"),
    inspector: required("[data-inspector]"),
    diagnostics: required("[data-diagnostics]"),
    undo: required("[data-undo]"),
    redo: required("[data-redo]"),
    save: required("[data-save]"),
    download: required("[data-download]")
  };
  var payload;
  var session;
  var view;
  var browserMeasurer;
  var browserMeasurementError;
  var selectedSlideId;
  var lastOperationDiagnostics = [];
  var fitLinesByObject = /* @__PURE__ */ new Map();
  var fileHandle;
  var lastSavedSha256;
  ...
  async function bootstrap() {
    ...
    const sourceBytes = decodeBase642(payload.sourceBase64);
    ...
    if ((session.state.sourceKind === "html" ? "deck" : "atom") !== payload.documentKind) {
    ...
    selectedSlideId = session.state.sourceKind === "html" ? session.state.deck.slideOrder[0] : void 0;
  ...
  function parsePayload(serialized) {
    ...
    if (value.schema !== "vector180-editor-pack/0.1" || value.documentKind !== "deck" && value.documentKind !== "atom" || typeof value.sourceSha256 !== "string" || !/^[0-9a-f]{64}$/u.test(value.sourceSha256) || typeof value.sourceBase64 !== "string" || typeof value.downloadName !== "string" || !Array.isArray(value.fonts) || typeof value.nearLimit !== "number" || !Number.isFinite(value.nearLimit) || value.nearLimit <= 0 || value.nearLimit >= 1) {
      throw new Error("Embedded editor payload is invalid.");
    }
    return value;
  }
  async function prepareBrowserMeasurement() {
    try {
      const sources = [];
      for (const font of payload.fonts) {
        const bytes = decodeBase642(font.sourceBase64);
      ...
      browserMeasurementError = error instanceof Error ? error.message : String(error);
      browserMeasurer = void 0;
    }
  }
  ...
  function recompute() {
    ...
    if (state.sourceKind === "html") {
      ...
      if (selectedSlideId === void 0 || !state.deck.slides.has(selectedSlideId)) {
  ...
  function browserMeasure(request) {
    if (browserMeasurer !== void 0) return browserMeasurer.measure(request);
    return {
      kind: "unverified",
      method: "browser-editor-unavailable",
      reason: browserMeasurementError ?? "No exact browser font measurement adapter is available.",
      missingCodepoints: []
    };
  }
  function combineFitEvidence(nodeLines, browserLines) {
    const remainingNode = [...nodeLines];
    const result = /* @__PURE__ */ new Map();
    for (const browser of browserLines) {
      const nodeIndex = remainingNode.findIndex(
        (candidate) => sameLineEvidence(candidate, browser)
      );
      const node = nodeIndex < 0 ? void 0 : remainingNode.splice(nodeIndex, 1)[0];
      const status = node === void 0 ? worseStatus("unverified", browser.status) : worseStatus(node.status, browser.status);
      const combined = {
        current: browser,
        ...node === void 0 ? {} : { node },
        browser,
        status,
        nodeCurrent: node !== void 0
      };
  ...
  function sameLineEvidence(left, right) {
    return left.objectId === right.objectId && left.lineIndex === right.lineIndex && left.text === right.text && left.anchor === right.anchor && left.anchorX === right.anchorX && left.frameX === right.frameX && left.frameWidth === right.frameWidth && left.availableWidth === right.availableWidth && left.font.family === right.font.family && left.font.size === right.font.size && left.font.weight === right.font.weight && left.font.style === right.font.style && ("slideId" in left ? "slideId" in right && left.slideId === right.slideId : "atomId" in right && left.atomId === right.atomId);
  }
  ...
  function render() {
    ...
    nodes.integrity.textContent = state.editable ? "source hash verified" : "source integrity failed — read only";
    ...
    nodes.save.hidden = getSaveFilePicker() === void 0;
    nodes.save.disabled = !state.editable;
    nodes.download.textContent = state.sourceKind === "html" ? "Download deck" : "Download diagram";
    renderNavigation();
    renderTree();
    renderViewport();
    renderTextEditor();
  ...
  function renderTree() {
    const objects = view.kind === "deck" ? view.inventory.slides.find(({ id }) => id === selectedSlideId)?.objects ?? [] : view.inventory.objects;
  ...
  function renderViewport() {
    const container = view.kind === "deck" ? view.resolved.slides.find(({ id }) => id === selectedSlideId) : view.resolved;
    if (container === void 0) {
    ...
    svg.setAttribute(
      "aria-label",
      view.kind === "deck" ? `Resolved slide ${selectedSlideId ?? ""}` : `Resolved diagram ${view.outline.atomId}`
    );
    const objects = view.kind === "deck" ? container.objects : view.resolved.objects;
  ...
  function objectFitStatus(objectId) {
    return (fitLinesByObject.get(objectId) ?? []).reduce(
  ...
  function renderTextEditor() {
    ...
    const directText = object?.role === "text" && object.directTextRange !== void 0;
    ...
    nodes.textHelp.textContent = object?.role !== "text" ? "Select one direct-text object." : directText ? "Apply commits one hash-bound C5 text replacement. No wrapping or repair occurs." : "This text has structured hard lines and is read-only in the direct-text editor.";
  }
  function renderInspector() {
    ...
    if (object === void 0) {
      ...
      values.push(
        ...
        [
          "Text",
          object.kind === "text" ? object.lines.map(({ text }) => text).join("\n") : "—"
        ]
      );
      ...
      for (const line of fit) {
        values.push(
          ...
          [
            ...
            line.nodeCurrent && line.node !== void 0 ? formatFitEvidence(line.node) : "unverified — embedded Fontkit evidence is stale for this edited line"
          ],
          [
    ...
    if (browserMeasurer !== void 0) {
      values.push(
        ...
        [
          "Exact browser fonts",
          browserMeasurer.fonts.length === 0 ? "none" : browserMeasurer.fonts.map(({ family, sha256 }) => `${family} ${sha256.slice(0, 12)}`).join(", ")
        ]
      );
    } else {
      values.push([
  ...
  function formatFitEvidence(line) {
    const width = line.measuredWidth === null ? "width unverified" : `${line.measuredWidth.toFixed(3)} / ${line.availableWidth.toFixed(3)}`;
    const identity = line.fontIdentity === void 0 ? "" : ` · ${line.fontIdentity}`;
    const reason = line.reason === void 0 ? "" : ` · ${line.reason}`;
  ...
  function renderDiagnostics() {
    ...
    const entries = [
      ...
      ...view.diagnostics,
      ...lastOperationDiagnostics,
      ...browserMeasurementError === void 0 ? [] : [
        {
    ...
    const visible = entries.length > 0 ? entries : [
      {
        code: "VECTOR180-EDITOR-READY",
        severity: "info",
        message: "Exact source is open through the shared C4/C5 session. Preview and projections are fresh C6 data."
      }
    ];
  ...
  function renderDeckControls() {
    ...
    nodes.theme.value = state.deck.themes.has(selectedTheme) ? selectedTheme : state.deck.activeTheme ?? "";
    nodes.theme.disabled = !state.editable;
    nodes.themeApply.disabled = !state.editable || nodes.theme.value === state.deck.activeTheme;
    const index = state.deck.slideOrder.indexOf(selectedSlideId ?? "");
    nodes.slideUp.disabled = !state.editable || index <= 0;
    nodes.slideDown.disabled = !state.editable || index < 0 || index >= state.deck.slideOrder.length - 1;
    nodes.extract.disabled = selectedSlideId === void 0;
  }
  async function applyDirectText() {
  ...
  async function moveSlide(delta) {
    const state = session.state;
    if (state.sourceKind !== "html" || selectedSlideId === void 0) return;
  ...
  async function dispatch(intent, successMessage) {
    ...
    announce(
      result.diagnostics[0]?.message ?? "The edit failed without changing source.",
      "error"
    );
    render();
  }
  async function extractSelectedSlide() {
    const state = session.state;
    if (state.sourceKind !== "html" || selectedSlideId === void 0) return;
    const result = await extractVector180Atom(state.deck, selectedSlideId);
    lastOperationDiagnostics = result.diagnostics;
    if (result.sourceText === void 0) {
      announce(
  ...
  async function saveCurrentSource() {
    ...
    if (picker === void 0 || !session.state.editable) return;
    try {
      if (fileHandle === void 0) {
        fileHandle = await picker({
          ...
          types: [
            {
              description: payload.documentKind === "deck" ? "Vector180 HTML deck" : "Vector180 SVG diagram",
              accept: {
                [payload.downloadMime.split(";")[0]]: [
                  payload.documentKind === "deck" ? ".vector180.html" : ".vector180.svg"
                ]
              }
            }
          ]
        });
      } else if (lastSavedSha256 !== void 0) {
  ...
  function getSaveFilePicker() {
    const candidate = globalThis.showSaveFilePicker;
    return typeof candidate === "function" ? candidate.bind(globalThis) : void 0;
  }
  ...
  function findSelectedNode(state) {
    const id = state.selectedId;
    if (id === void 0) return void 0;
    const roots = state.sourceKind === "html" ? [...state.deck.slides.values()].flatMap(({ children }) => children) : state.atom.children;
    return findNode3(roots, id);
  }
  function findNode3(roots, id) {
  ...
  function findResolvedObject(id) {
    if (id === void 0) return void 0;
    const roots = view.kind === "deck" ? view.resolved.slides.find(({ id: id2 }) => id2 === selectedSlideId)?.objects ?? [] : view.resolved.objects;
    return findResolved(roots, id);
  }
  function findResolved(roots, id) {
  ...
  function disableWritableControls() {
  ...
  function svgElement(name) {
    return document.createElementNS(SVG_NAMESPACE3, name);
  }
