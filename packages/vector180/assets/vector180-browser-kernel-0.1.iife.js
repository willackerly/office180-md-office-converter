/*! @office180/vector180 browser kernel 0.1 | generated by esbuild 0.28.1 */
"use strict";
var Vector180BrowserKernel = (() => {
...
`;
  ...
  __export(conformance_entry_exports, {
    ...
    VECTOR180_ATOM_DISCOVERY_COMMENT: () => VECTOR180_ATOM_DISCOVERY_COMMENT,
    Vector180LoadError: () => Vector180LoadError,
    addVector180AtomDiscoveryComment: () => addVector180AtomDiscoveryComment,
    applyPatch: () => applyPatch,
    atomIsValid: () => atomIsValid,
    browserEnvironmentFromUserAgent: () => browserEnvironmentFromUserAgent,
    browserFontAlias: () => browserFontAlias,
    compareAtomMetadata: () => compareAtomMetadata,
    deckIsValid: () => deckIsValid,
    deriveStylePaletteSha256: () => deriveStylePaletteSha2562,
    diffVector180Atoms: () => diffVector180Atoms,
    extractAtomText: () => extractAtomText,
    extractText: () => extractText,
    extractVector180Atom: () => extractVector180Atom,
    getAtom: () => getAtom,
    getAtomObject: () => getAtomObject,
    getObject: () => getObject,
    getSlide: () => getSlide,
    hasErrors: () => hasErrors,
    inspectVector180Conformance: () => inspectVector180Conformance,
    inventoryAtom: () => inventoryAtom,
    inventoryDeck: () => inventoryDeck,
    loadAtom: () => loadAtom,
    loadDeck: () => loadDeck,
    loadVector180Document: () => loadVector180Document,
    materializeSource: () => materializeSource,
    migratePptvAtom: () => migratePptvAtom,
    outlineAtom: () => outlineAtom,
    outlineDeck: () => outlineDeck,
    outlineManifest: () => outlineManifest,
    parseManifest: () => parseManifest,
    preflightAtomTextFit: () => preflightAtomTextFit,
    preflightDeckTextFit: () => preflightDeckTextFit,
    prepareVector180BrowserTextMeasurer: () => prepareVector180BrowserTextMeasurer,
    projectAtomMetadata: () => projectAtomMetadata,
    queryAtomObjects: () => queryAtomObjects,
    queryObjects: () => queryObjects,
    resolveVector180Atom: () => resolveVector180Atom,
    resolveVector180AtomStyles: () => resolveVector180AtomStyles,
    resolveVector180Deck: () => resolveVector180Deck,
    resolveVector180Styles: () => resolveVector180Styles,
    scanVector180Source: () => scanVector180Source,
    sha256Hex: () => sha256Hex,
  ...
  // src/core/dialect.ts
  var VECTOR180_DIALECT = Object.freeze({
  ...
  var PPTV_LEGACY_DIALECT = Object.freeze({
  ...
  var RESERVED_ATTRIBUTE_SUFFIXES = [
    "version",
    "role",
    "export",
    "from",
  ...
  var RESERVED_ATTRIBUTE_PATTERN = new RegExp(
    String.raw`\bdata-(vector180|pptv)-(?:${RESERVED_ATTRIBUTE_SUFFIXES.join("|")})\b`,
    "giu"
  );
  function dialectFor(family) {
    return family === "vector180" ? VECTOR180_DIALECT : PPTV_LEGACY_DIALECT;
  }
  function wireAttribute(family, suffix) {
    return `data-${dialectFor(family).prefix}-${suffix}`;
  }
  function wireCssTokenPrefix(family) {
    return `--${dialectFor(family).prefix}-`;
  }
  function detectWireDialect(kind, sourceText) {
    const vector180Evidence = /* @__PURE__ */ new Set();
    const legacyEvidence = /* @__PURE__ */ new Set();
    ...
    for (const tag of withoutComments.matchAll(/<[^!?][^>]*>/gu)) {
      ...
      if (/\btype\s*=\s*["']application\/vnd\.office180\.vector180\+json["']/iu.test(
        tagText
      )) {
      ...
      if (/\bname\s*=\s*["']vector180-agent-profile["']/iu.test(tagText) || /\bcontent\s*=\s*["']vector180-agent\/1["']/iu.test(tagText)) {
        vector180Evidence.add("vector180-agent/1");
      }
      if (/\bname\s*=\s*["']pptv-agent-profile["']/iu.test(tagText) || /\bcontent\s*=\s*["']pptv-agent\/1["']/iu.test(tagText)) {
    ...
    const dialect = mixed || vector180Evidence.size === 0 && legacyEvidence.size === 0 ? void 0 : vector180Evidence.size > 0 ? VECTOR180_DIALECT : PPTV_LEGACY_DIALECT;
    return {
  ...
  function addProfileEvidence(json, vector180Evidence, legacyEvidence) {
    if (/"(?:runtime|editor|agentProfile)"\s*:\s*"vector180-(?:browser|editor|agent)\/[0-9.]+"/u.test(
      json
    )) {
      vector180Evidence.add("vector180 profile");
    }
    if (/"(?:runtime|editor|agentProfile)"\s*:\s*"pptv-(?:browser|editor|agent)\/[0-9.]+"/u.test(
      json
    )) {
      legacyEvidence.add("pptv profile");
    }
  }
  function stripMarkupComments(sourceText) {
    return sourceText.replace(/<!--[\s\S]*?-->/gu, "");
  }
  function addEvidence(family, marker, vector180Evidence, legacyEvidence) {
    ...
    if (input.kind === "bytes") {
      ...
      if (!isWellFormedUtf16(text)) {
        diagnostics.push({
          code: "VECTOR180-SCAN-INVALID-UTF8",
          severity: "fatal",
          message: "Vector180 text input must not contain unpaired UTF-16 surrogates."
        });
      }
  ...
  function sha256HexSync(bytes) {
    ...
    for (let block = 0; block < padded.length; block += 64) {
      for (let index = 0; index < 16; index += 1) {
        ...
        words[index] = ((padded[offset] ?? 0) << 24 | (padded[offset + 1] ?? 0) << 16 | (padded[offset + 2] ?? 0) << 8 | (padded[offset + 3] ?? 0)) >>> 0;
      }
  ...
  function parseManifest(scan) {
        ...
        diagnostics: [
          {
            code: "VECTOR180-MANIFEST-INVALID",
            severity: "error",
            message: "Vector180 source does not contain a source-located manifest value."
          }
        ]
      };
    }
    ...
    if (dialect.manifestField === "pptv" && ranges.fields.has("pptv") && !ranges.fields.has("vector180")) {
  ...
  function validateManifest(manifest, scan) {
    const diagnostics = [];
    const dialect = scan.wireFamily === void 0 ? void 0 : dialectFor(scan.wireFamily);
    ...
    if (scan.kind !== "html") {
      diagnostics.push({
        ...
        severity: "error",
        message: "Vector180 0.1 semantic loading currently supports self-contained .vector180.html sources only."
      });
      return diagnostics;
    }
    if (dialect === void 0) {
    ...
    if (scan.versionHint !== void 0 && scan.versionHint !== manifest.vector180) {
      diagnostics.push({
    ...
    if (manifest.editor !== void 0 && manifest.editor !== dialect.editorRuntime) {
      diagnostics.push({
        code: "VECTOR180-NAMESPACE-MIXED",
        severity: "error",
    ...
    if (manifest.agentProfile !== void 0 && manifest.agentProfile !== dialect.agentProfile) {
      diagnostics.push({
        code: "VECTOR180-NAMESPACE-MIXED",
        severity: "error",
    ...
    if (manifest.theme === void 0 || (themeSections.get(manifest.theme)?.length ?? 0) !== 1) {
      diagnostics.push({
        code: "VECTOR180-MANIFEST-MISSING-REFERENCE",
        severity: "error",
        message: manifest.theme === void 0 ? "Self-contained Vector180 HTML requires one active manifest theme." : `Active theme "${manifest.theme}" does not resolve to exactly one theme declaration.`
      });
    }
    ...
    if (manifest.runtime === void 0 || (runtimeSections.get(manifest.runtime)?.length ?? 0) !== 1) {
      diagnostics.push({
        code: "VECTOR180-MANIFEST-MISSING-REFERENCE",
        severity: "error",
        message: manifest.runtime === void 0 ? "Self-contained Vector180 HTML requires a manifest runtime identifier." : `Runtime "${manifest.runtime}" does not resolve to the fixed viewer runtime declaration.`
      });
    }
  ...
  function validateManifestValue(value, root, baseOffset, mapper, manifestField) {
    const diagnostics = [];
    if (!isRecord(value)) return diagnostics;
    const manifestFields = /* @__PURE__ */ new Set([
      manifestField,
      "title",
      "runtime",
      "editor",
    ...
    while (pending.length > 0) {
      ...
      if (count > MAX_MANIFEST_NODES || current.depth > MAX_MANIFEST_DEPTH) {
        return {
          code: "VECTOR180-MANIFEST-LIMIT",
          severity: "fatal",
          message: count > MAX_MANIFEST_NODES ? `Vector180 manifest exceeds the ${MAX_MANIFEST_NODES} node limit.` : `Vector180 manifest exceeds the ${MAX_MANIFEST_DEPTH} nesting-depth limit.`,
  ...
  function normalizeManifestValue(value, manifestField) {
    if (!isRecord(value) || typeof value[manifestField] !== "string") {
      return void 0;
    }
    if (value.title !== void 0 && typeof value.title !== "string" || value.runtime !== void 0 && typeof value.runtime !== "string" || value.editor !== void 0 && typeof value.editor !== "string" || value.theme !== void 0 && typeof value.theme !== "string" || value.themes !== void 0 && (!Array.isArray(value.themes) || !value.themes.every((theme) => typeof theme === "string")) || !Array.isArray(value.slides) || !value.slides.every(isManifestSlideValue) || value.agentProfile !== void 0 && typeof value.agentProfile !== "string" || value.extensions !== void 0 && !isRecord(value.extensions)) {
      return void 0;
    }
    return {
      vector180: value[manifestField],
      ...value.title === void 0 ? {} : { title: value.title },
      ...value.runtime === void 0 ? {} : { runtime: value.runtime },
      ...value.editor === void 0 ? {} : { editor: value.editor },
      ...value.theme === void 0 ? {} : { theme: value.theme },
      ...value.themes === void 0 ? {} : { themes: value.themes },
      slides: value.slides,
      ...value.agentProfile === void 0 ? {} : { agentProfile: value.agentProfile },
      ...value.extensions === void 0 ? {} : { extensions: value.extensions }
    };
  }
  ...
  var METADATA_SCHEMA = "vector180-atom-metadata/0.1";
  var MAX_METADATA_BYTES = 4096;
  var MAX_METADATA_DEPTH = 4;
  var SHA256_PATTERN = /^[a-f0-9]{64}$/u;
  var VERSION_PATTERN = /^[0-9]+(?:\.[0-9]+)*$/u;
  async function parseAtomMetadata(input) {
    const diagnostics = [];
    ...
    const parseErrors = [];
    const root = parseTree2(input.payload, parseErrors, {
      allowTrailingComma: false,
      disallowComments: true,
      allowEmptyContent: false
    });
    ...
    diagnostics.push(...detectDuplicateKeys2(root, input.contentRange));
    ...
    if (metadata === void 0 || hasMetadataErrors(diagnostics)) {
      return { diagnostics };
    }
  ...
  function validateMetadata(value, diagnostics, range) {
    if (!isRecord2(value)) {
      diagnostics.push(
    ...
    if (value.hydration === void 0 && value.templateLineage === void 0 && value.styleFamily === void 0) {
      diagnostics.push(
        invalidMetadata(
    ...
    const hydration = value.hydration === void 0 ? void 0 : validateHydration(value.hydration, diagnostics, range);
    const templateLineage = value.templateLineage === void 0 ? void 0 : validateTemplateLineage(value.templateLineage, diagnostics, range);
    const styleFamily = value.styleFamily === void 0 ? void 0 : validateStyleFamily(value.styleFamily, diagnostics, range);
    if (value.hydration !== void 0 && hydration === void 0 || value.templateLineage !== void 0 && templateLineage === void 0 || value.styleFamily !== void 0 && styleFamily === void 0) {
      return void 0;
    }
    return {
      ...hydration === void 0 ? {} : { hydration },
      ...templateLineage === void 0 ? {} : { templateLineage },
      ...styleFamily === void 0 ? {} : { styleFamily }
    };
  }
  function validateHydration(value, diagnostics, range) {
    if (!isRecord2(value)) {
    ...
    const valid = isProfile(value.method) && (value.sourceWireFamily === "vector180" || value.sourceWireFamily === "pptv-legacy") && isSha256(value.sourceSha256) && isStableId(value.sourceObjectId) && isSha256(value.sourceObjectSha256) && isStableId(value.activeThemeId) && keys.every((key) => Object.hasOwn(value, key));
    if (!valid) {
      diagnostics.push(
        invalidMetadata(
    ...
    return {
      method: value.method,
      sourceWireFamily: value.sourceWireFamily,
      sourceSha256: value.sourceSha256,
      sourceObjectId: value.sourceObjectId,
      sourceObjectSha256: value.sourceObjectSha256,
      activeThemeId: value.activeThemeId
    };
  }
  function validateTemplateLineage(value, diagnostics, range) {
    if (!isRecord2(value)) {
      diagnostics.push(
    ...
    if (!isProfile(value.generatorProfile) || !isStableId(value.templateId) || !isSha256(value.templateSha256) || !keys.every((key) => Object.hasOwn(value, key))) {
      diagnostics.push(
        invalidMetadata(
  ...
  function validateStyleFamily(value, diagnostics, range) {
    if (!isRecord2(value)) {
    ...
    if (!isStableId(value.id) || value.version !== void 0 && (typeof value.version !== "string" || value.version.length > 32 || !VERSION_PATTERN.test(value.version)) || value.definitionSha256 !== void 0 && !isSha256(value.definitionSha256)) {
      diagnostics.push(
        invalidMetadata(
    ...
    return {
      ...
      ...value.definitionSha256 === void 0 ? {} : { definitionSha256: value.definitionSha256 }
    };
  }
  function rejectUnknownKeys(value, allowed, label, diagnostics, range) {
  ...
  function detectDuplicateKeys2(root, range) {
    const diagnostics = [];
    const pending = [root];
    while (pending.length > 0) {
      const node = pending.pop();
  ...
  function jsonDepth(root) {
    ...
    while (pending.length > 0) {
      ...
      for (const child of current.node.children ?? []) {
        pending.push({
          node: child,
          depth: child.type === "object" || child.type === "array" ? current.depth + 1 : current.depth
        });
      }
    }
    return maximum;
  ...
  function invalidMetadata(message, range) {
    return {
  ...
  function hasMetadataErrors(diagnostics) {
    return diagnostics.some(
      (diagnostic) => diagnostic.severity === "error" || diagnostic.severity === "fatal"
    );
  }
  ...
  async function scanVector180Source(input, options = {}) {
    ...
    const document = detection.dialect === void 0 ? materialized.document : sourceWithWireFamily(
    ...
    if (detection.mixed) {
      diagnostics.push({
        code: "VECTOR180-NAMESPACE-MIXED",
        severity: "fatal",
        message: "Source mixes canonical Vector180 and legacy PPTV control vocabularies; dialect selection is whole-document.",
        related: [
    ...
    } else if (kind === "unknown" || detection.dialect === void 0) {
      diagnostics.push({
        code: "VECTOR180-SCAN-UNRECOGNIZED",
        severity: "fatal",
        message: "Source is not a recognizable canonical Vector180 or legacy PPTV HTML, SVG, or manifest document."
      });
    }
    if (kind === "unknown" || detection.dialect === void 0) {
      return {
        kind,
        encoding: "utf-8",
        source: document,
  ...
  function validateStandaloneSvgXml(source, mapper, diagnostics) {
    ...
    parser.on("xmldecl", (declaration) => {
      if (declaration.version !== void 0 && declaration.version !== "1.0") {
        policyFailure = "Standalone Vector180 SVG requires XML 1.0 when an XML declaration is present.";
        throw new Error(policyFailure);
      }
    });
    parser.on("doctype", () => {
      policyFailure = "Standalone Vector180 SVG forbids DOCTYPE, DTD, and custom entity declarations.";
      throw new Error(policyFailure);
    });
    try {
      ...
      diagnostics.push({
        ...
        severity: "fatal",
        message: policyFailure ?? `Standalone Vector180 SVG is not namespace-aware XML 1.0: ${error instanceof Error ? error.message : "unknown XML parse error"}`,
        range: mapper.range(position, Math.min(source.text.length, position + 1))
      });
      return false;
    }
  }
  async function scanHtml(source, mapper, diagnostics, options, dialect) {
    const parseErrors = [];
    ...
    if (!hasExplicitPairedElement(html) || !hasExplicitPairedElement(head) || !hasExplicitPairedElement(body)) {
      diagnostics.push({
        ...
        severity: "fatal",
        message: "Vector180 HTML requires explicit, source-located html, head, and body elements."
      });
    } else {
          ...
          if (!isElement(child)) {
            if (!isInertWhitespaceOrComment(child)) {
              ...
              diagnostics.push({
                ...
                severity: "error",
                message: "Unexpected visible text at top level in the Vector180 body.",
                ...location == null ? {} : {
                  range: mapper.range(
    ...
    const htmlVersion = html === void 0 ? void 0 : getAttribute(html, dialect.versionAttribute);
  ...
  function scanSvg(source, mapper, diagnostics, options, dialect) {
    const parseErrors = [];
    const sourceOffset = source.text.startsWith("\uFEFF") ? 1 : 0;
    ...
    if (svg === void 0 || svg.sourceCodeLocation == null || hasCompetingContent) {
      diagnostics.push({
        code: "VECTOR180-SCAN-UNRECOGNIZED",
        severity: "fatal",
        message: "Standalone Vector180 SVG must contain one source-located root svg element."
      });
      return {
        kind: "svg",
        encoding: "utf-8",
  ...
  function rejectDeckAtomMetadataMarkers(html, mapper, diagnostics, sourceOffset) {
    walk(html, (node) => {
      ...
      for (const marker of [
        ...
        if (getAttribute(node, marker) === void 0) continue;
        const location = findAttributeLocation(node, marker);
        diagnostics.push({
          code: "VECTOR180-METADATA-INVALID",
          severity: "error",
          message: "Atom metadata markers are unsupported anywhere inside deck HTML.",
          ...location === void 0 ? {} : {
            range: mapper.range(
              sourceOffset + location.startOffset,
              sourceOffset + location.endOffset
  ...
  function validateAtomMetadataMarkers(svg, mapper, diagnostics, sourceOffset, dialect) {
    walk(svg, (node) => {
      if (!isElement(node)) return;
      const legacyMarker = getAttribute(node, "data-pptv-metadata");
      const canonicalMarker = getAttribute(node, "data-vector180-metadata");
      const invalidLegacy = legacyMarker !== void 0;
      const invalidCanonicalPlacement = canonicalMarker !== void 0 && (dialect.wireFamily !== "vector180" || node.tagName !== "metadata" || node.parentNode !== svg);
      if (!invalidLegacy && !invalidCanonicalPlacement) return;
      const marker = invalidLegacy ? "data-pptv-metadata" : "data-vector180-metadata";
      const location = findAttributeLocation(node, marker);
      diagnostics.push({
        code: "VECTOR180-METADATA-INVALID",
        severity: "error",
        message: invalidLegacy ? "Legacy PPTV 0.1 has no structured metadata marker; data-pptv-metadata is forbidden." : "Canonical atom metadata must be a direct-child metadata element.",
        ...location === void 0 ? {} : {
          range: mapper.range(
            sourceOffset + location.startOffset,
            sourceOffset + location.endOffset
  ...
  function validateAtomStyleAuthority(svg, mapper, diagnostics, sourceOffset, dialect) {
    walk(svg, (node) => {
      if (!isElement(node)) return;
      if (node.tagName === "style") {
        ...
        diagnostics.push({
          ...
          severity: "error",
          message: "Standalone Vector180 diagrams forbid style elements; use local presentation attributes or inline style.",
          ...range === void 0 ? {} : { range }
        });
      }
      for (const attribute of node.attrs) {
        ...
        const reason = normalizedName === "class" ? "class attributes because no class stylesheet authority exists" : normalizedName === wireAttribute(dialect.wireFamily, "style") || normalizedName === wireAttribute(dialect.wireFamily, "theme") ? `diagram control attribute "${name}"` : normalizedName.startsWith("--") || /var\s*\(/iu.test(attribute.value) || attribute.value.includes(
          wireCssTokenPrefix(dialect.wireFamily)
        ) || normalizedName === "style" && /(?:^|;)\s*--[-_A-Za-z0-9]+\s*:/u.test(attribute.value) ? `custom-property or var() styling in attribute "${name}"` : void 0;
  ...
  function validateAtomRoot(svg, mapper, diagnostics, sourceOffset, dialect) {
    ...
    if (id === void 0 || !STABLE_ID_PATTERN.test(id)) {
      diagnostics.push({
        ...
        severity: "error",
        message: "Standalone Vector180 SVG requires an explicit valid stable ID on its root; filenames never supply semantic identity.",
    ...
    if (parseAtomViewBox(getAttribute(svg, "viewBox")) === void 0) {
      diagnostics.push({
        code: "VECTOR180-SVG-VIEWBOX",
        severity: "error",
        message: "Standalone Vector180 SVG requires a finite four-number viewBox with positive width and height.",
        ...rootRange === void 0 ? {} : { range: rootRange }
      });
    }
    const diagramRootAttributes = /* @__PURE__ */ new Set([
      "id",
  ...
  function parseAtomViewBox(value) {
  ...
  function classifyBodyElement(element, mapper, diagnostics, sourceOffset, dialect) {
    ...
    const slideId2 = getAttribute(element, slideAttribute);
    ...
    const markerKinds = [
      ...manifestId === dialect.manifestId && type === dialect.manifestType ? ["manifest"] : [],
    ...
    if (element.tagName === "script" && manifestId === dialect.manifestId && type === dialect.manifestType) {
    ...
    if (element.tagName !== "script" && hasAttribute(element, outputAttribute)) {
      if (element.tagName !== "main" && element.tagName !== "div" || !hasOnlyAttributes(element, [outputAttribute]) || element.childNodes.some((child) => !isInertWhitespaceOrComment(child))) {
        const range2 = locationRange(element, mapper, sourceOffset);
        diagnostics.push({
          code: "VECTOR180-SCAN-OUTPUT-NONEMPTY",
          severity: "error",
          message: "The Vector180 output mount must be an empty, inert main or div element.",
          ...range2 === void 0 ? {} : { range: range2 }
        });
      }
      return sectionFromElement(
    ...
    if (element.tagName === "script" && type === "text/css" && styleId !== void 0) {
      ...
      if (styleId !== "base") {
        ...
        diagnostics.push({
          ...
          severity: "error",
          message: 'The strict Vector180 base-style identifier must be exactly "base".',
          ...range2 === void 0 ? {} : { range: range2 }
        });
      }
    ...
    if (element.tagName === "script") {
      ...
      diagnostics.push({
        ...
        severity: "error",
        message: "Strict Vector180 HTML permits only the manifest, inert base/theme CSS, and recognized runtimes.",
        ...range2 === void 0 ? {} : { range: range2 }
      });
      return sectionFromElement(
        "unknown",
  ...
  function validateHead(head, mapper, diagnostics, sourceOffset, dialect) {
    let titleCount = 0;
  ...
  function validateContainerAttributes(html, head, body, mapper, diagnostics, sourceOffset, dialect) {
    const declarations = [
    ...
    walk(root, (node) => {
      ...
      if (node.tagName === "script" && (node.parentNode === null || !isElement(node.parentNode) || node.parentNode.tagName !== "body")) {
        ...
        diagnostics.push({
          code: "VECTOR180-SECURITY-EXECUTABLE",
          severity: "error",
          message: "Executable script elements are forbidden outside the recognized top-level Vector180 control blocks.",
          ...range === void 0 ? {} : { range }
        });
      }
      for (const attribute of node.attrs) {
        ...
        if (node.tagName === "script" && qualifiedName.toLowerCase() === "src") {
          diagnostics.push({
            code: "VECTOR180-SECURITY-EXECUTABLE",
            severity: "error",
            message: "Vector180 runtime and data scripts must be inline; src is forbidden.",
            ...range === void 0 ? {} : { range }
          });
        }
        if (qualifiedName.toLowerCase() === "srcset") {
          diagnostics.push({
            ...
            severity: "error",
            message: "srcset is forbidden until Vector180 has a deterministic resource-set resolver.",
            ...range === void 0 ? {} : { range }
          });
        }
  ...
  function unsafeUrlReason(value) {
    ...
    return value.startsWith("#") ? void 0 : "Only same-document fragment references are allowed in self-contained Vector180 0.1";
  }
  async function validateReferenceRuntimes(source, sections, diagnostics, dialect) {
    for (const section of sections) {
      if (section.kind !== "viewer-runtime" && section.kind !== "editor-runtime") {
        continue;
      }
      const expectedDigest = section.kind === "viewer-runtime" && section.id !== void 0 ? section.id === dialect.browserRuntime ? REFERENCE_RUNTIME_DIGESTS.get(section.id) : void 0 : void 0;
  ...
  function unsafeCssResourceReason(css) {
    ...
    for (const match of urls) {
      ...
      return unsafeUrlReason(value) ?? "External CSS resources are disabled in self-contained Vector180 0.1";
    }
    return void 0;
  }
  function sourceWithWireFamily(source, wireFamily) {
    ...
    while (pending.length > 0) {
      ...
      if (elements > maxElements || current.depth > maxDepth) {
        ...
        diagnostics.push({
          ...
          severity: "fatal",
          message: elements > maxElements ? `Vector180 source exceeds the ${maxElements} element limit.` : `Vector180 source exceeds the ${maxDepth} nesting-depth limit.`,
          ...range === void 0 ? {} : { range }
        });
        return;
      }
  ...
  var Vector180LoadError = class extends Error {
    diagnostics;
  ...
  async function loadAtom(input, options = {}) {
    const scan = await scanVector180Source(input, options);
    assertSafelyScanned(scan);
    if (scan.kind !== "svg") {
  ...
  async function loadVector180Document(input, options = {}) {
    const scan = await scanVector180Source(input, options);
    assertSafelyScanned(scan);
  ...
  async function loadAtomFromScan(scan) {
    ...
    const dialect = dialectFor(scan.wireFamily);
    const version = root === void 0 ? void 0 : getAttribute2(root, dialect.versionAttribute);
    ...
    const metadataElements = root.childNodes.filter(
      (child) => isElement2(child) && child.tagName === "metadata" && getAttribute2(child, metadataAttribute) !== void 0
    );
    ...
    if (metadataElements.length > 1) {
      diagnostics.push({
        code: "VECTOR180-METADATA-INVALID",
        severity: "error",
        message: "A Vector180 atom permits at most one recognized direct-child metadata element.",
        range: metadataElements[1]?.sourceCodeLocation == null ? rootSection.range : offsetRange(
      ...
      const contentStart = location.startTag?.endOffset;
      const contentEnd = location.endTag?.startOffset;
      const hasUnsupportedShape = metadataElement.attrs.length !== 1 || metadataElement.attrs[0] === void 0 || qualifiedAttributeName2(metadataElement.attrs[0]).toLowerCase() !== metadataAttribute || contentStart === void 0 || contentEnd === void 0 || !metadataElement.childNodes.every(isText);
      if (hasUnsupportedShape) {
        diagnostics.push({
          code: "VECTOR180-METADATA-INVALID",
          severity: "error",
          message: "Recognized atom metadata must contain only its marker attribute and one inert JSON text payload.",
        ...
        const parsedMetadata = await parseAtomMetadata({
          marker: getAttribute2(metadataElement, metadataAttribute),
  ...
  function parseSlideSection(slideIdValue, manifestSlide, contentRange, sectionRange, sourceText, mapper, declarations, wireFamily) {
    const diagnostics = [];
    ...
    const svgLayout = getAttribute2(svg, wireAttribute(wireFamily, "layout"));
  ...
  function parseObject(element, scope, parentId, baseOffset, mapper, declarations, diagnostics, objectIds, indexedObjects) {
    ...
    const roleValue = getAttribute2(element, roleAttribute);
    const exportValue = getAttribute2(element, exportAttribute);
  ...
  function freezeDeck(deck) {
    ...
    const index = {
      ...
      runtimes: deepFreeze2([...deck.index.runtimes])
    };
    return deepFreeze2({
      ...deck,
      ...
      index: deepFreeze2(index),
      manifest: deepFreeze2(deck.manifest),
      materialization: {
  ...
  function immutableMap(values) {
    ...
    for (const [key, value] of values) {
      entries.set(key, deepFreeze2(value));
    }
  ...
  function resolveVector180Styles(deck) {
    const diagnostics = [];
    ...
    if (deck.activeTheme === void 0 || activeTheme === void 0) {
      diagnostics.push(
        makeDiagnostic(
          ...
          deck.activeTheme === void 0 ? "Vector180 style resolution requires one active theme." : `Active theme "${deck.activeTheme}" is not available for style resolution.`,
  ...
  function resolveVector180AtomStyles(diagram) {
    const diagnostics = [];
    const styles = /* @__PURE__ */ new Map();
    resolveNodes(
      diagram,
  ...
  function parseTheme(id, cssText, range, wireFamily) {
  ...
  function resolveNodeStyle(document, scopeId, node, rules, activeTheme, diagnostics) {
    ...
    if (document.sourceKind === "svg") {
      ...
      for (const [authoredName, value] of Object.entries(node.attributes)) {
        ...
        const prohibited = name === "class" || name === styleAttribute || name === themeAttribute || name.startsWith("--") || /var\s*\(/iu.test(value) || value.includes(tokenPrefix) || name === "style" && /(?:^|;)\s*--[-_A-Za-z0-9]+\s*:/u.test(value);
  ...
  function diagnosticScope(document, scopeId) {
    return document.sourceKind === "html" ? { slideId: scopeId } : { atomId: scopeId };
  }
  ...
  var SVG_ASSET_ATTRIBUTES = boundaryAttributes(
  ...
  function resolveVector180Deck(deck) {
  ...
  function resolveVector180Atom(atom) {
    ...
    if (!validAtomBaseSnapshot(atom)) {
      diagnostics.push({
        code: "VECTOR180-PROFILE-INVALID-BASE",
        severity: "error",
        message: "Atom resolution requires a complete, unambiguous standalone semantic snapshot with no errors.",
        atomId: atom.id
      });
      return { diagnostics };
    }
    ...
    const model = {
      ...
      atomId: atom.id,
      ...atom.metadata === void 0 || atom.metadataSha256 === void 0 || atom.index.metadata === void 0 ? {} : {
        metadata: {
          value: atom.metadata,
  ...
  function deriveStylePaletteSha256(objects) {
    ...
    visitResolvedAtomObjects(objects, (object) => {
      ...
      tuples.add(
        JSON.stringify([
          ...
          object.style.fontSize === void 0 ? null : canonicalNumber(object.style.fontSize),
          object.style.fontFamily === void 0 ? null : object.style.fontWeight,
          object.style.fontFamily === void 0 ? null : object.style.fontStyle
        ])
      );
    });
  ...
  function visitResolvedAtomObjects(objects, visitor) {
  ...
  function validAtomBaseSnapshot(diagram) {
    ...
    visitSourceNodes(diagram.children, (node) => {
      ...
      if (seen.has(node.id) || indexed === void 0 || indexed.atomId !== diagram.id) {
        valid = false;
      }
      seen.add(node.id);
    });
  ...
  function validAtomCanvas(viewBox) {
  ...
  function toAtomObject(object) {
    ...
    const { containerId, ...fields } = object;
    return { ...fields, atomId: containerId };
  }
  ...
  function resolveLine(node, scope, parentId, order, worldOffset, styleEntry, context) {
    ...
    const worldBounds = offsetBounds(localBounds, worldOffset, node, context);
    if (worldBounds === void 0) return void 0;
    const fromId = getAttribute4(
      node,
      wireAttribute(context.document.wireFamily, "from")
    );
    const toId = getAttribute4(
      node,
  ...
  function resolveText(node, scope, parentId, order, worldOffset, styleEntry, context) {
    ...
    const frame = parseTupleBounds(
      getAttribute4(node, frameAttribute),
      "VECTOR180-PROFILE-TEXT-FRAME",
      "text frame",
      node,
      context
    );
    const lineStepSource = getAttribute4(node, lineStepAttribute);
  ...
  function validateBoundaryAttributes(node, allowed, context) {
    for (const authoredName of Object.keys(node.attributes)) {
      ...
      pushNodeDiagnostic(
        context,
        node,
        styleOrLayout ? "VECTOR180-PROFILE-CSS-PROPERTY" : "VECTOR180-PROFILE-OBJECT-KIND",
  ...
  function parseDeclaredBounds(node, context) {
    return parseTupleBounds(
      getAttribute4(node, wireAttribute(context.document.wireFamily, "bounds")),
  ...
  // src/core/extract.ts
  var VECTOR180_ATOM_DISCOVERY_COMMENT = [
    "<!--",
  ...
  async function extractVector180Atom(deck, slideId2) {
    const diagnostics = [];
    if (hasErrors(deck.diagnostics) || !deck.materialization.complete) {
      diagnostics.push({
        code: "VECTOR180-EXTRACT-INVALID-BASE",
        severity: "error",
        message: "Atom extraction requires a complete C4 deck snapshot with no errors.",
        slideId: slideId2
      });
    ...
    try {
      atom = await loadAtom({
        kind: "text",
        text: sourceText,
        name: `${slideId2}.vector180.svg`
      });
    } catch (error) {
      const related = error instanceof Vector180LoadError ? error.diagnostics : [];
      diagnostics.push(...related, {
        code: "VECTOR180-EXTRACT-INVALID-CANDIDATE",
        severity: "error",
        message: "The hydrated source did not reload as an independent Vector180 diagram.",
        slideId: slideId2
      });
      return freezeResult({ provenance, diagnostics });
    }
    if (hasErrors(atom.diagnostics)) {
      diagnostics.push(...atom.diagnostics, {
        code: "VECTOR180-EXTRACT-INVALID-CANDIDATE",
        severity: "error",
        message: "The hydrated source did not validate as an independent Vector180 diagram.",
        slideId: slideId2
      });
      return freezeResult({ provenance, diagnostics });
    }
  ...
  function renameLegacyControlAttributes(source, ranges, edits) {
  ...
  function addHydrationMetadata(source, provenance) {
    ...
    const metadata = `
  <metadata data-vector180-metadata="vector180-atom-metadata/0.1">${payload}</metadata>`;
    return source.slice(0, openTagEnd + 1) + metadata + source.slice(openTagEnd + 1);
  }
  ...
  function assertRangeSpelling(source, range, name) {
    ...
    const pattern = new RegExp(`^${escapeRegExp2(name)}(?:\\s*=|\\s|$)`, "iu");
  ...
  function addVector180AtomDiscoveryComment(source) {
  ...
  function freezeResult(result) {
    return Object.freeze({
      ...result,
      ...result.provenance === void 0 ? {} : { provenance: Object.freeze({ ...result.provenance }) },
  ...
  function preflightDeckTextFit(deck, measurer, options = {}) {
  ...
  function preflightAtomTextFit(diagram, measurer, options = {}) {
  ...
  function preflightAtomLine(object, line, lineIndex, nearLimit, measurer) {
    const evidence = preflightLineEvidence(
      object,
      line,
      lineIndex,
  ...
  function visitAtomTextObjects(objects, visit2) {
    for (const object of objects) {
      if (object.kind === "text") visit2(object);
      else if (object.kind === "group")
        visitAtomTextObjects(object.children, visit2);
    }
  }
  ...
  async function applyPatch(document, input) {
    ...
    try {
      ...
      const cloneDiagnostics = cloneOperation !== void 0 && candidateResolution.state !== void 0 ? validateCloneCandidate(
        candidateDocument,
      ...
      return {
        ...
        ...candidateDocument.sourceKind === "html" ? { deck: candidateDocument } : { atom: candidateDocument },
  ...
  function planPatch(document, input) {
    ...
    if (document.wireFamily !== "vector180") {
      diagnostics.push({
        ...
        severity: "error",
        message: "Canonical patching accepts Vector180 source only; migrate a legacy PPTV atom first."
      });
    }
        ...
        diagnostics: [
          {
            ...
            ...error instanceof Vector180LoadError && error.diagnostics.length > 0 ? {
  ...
  function resolvePatchState(document) {
    const result = document.sourceKind === "html" ? resolveVector180Deck(document) : resolveVector180Atom(document);
    ...
    for (const [index, operation] of operations.entries()) {
      ...
      if (operation.op === "delete-object") {
        ...
        if (directChildIds.has(operation.id) || [...protectedIds].some((id) => deletedIds.has(id))) {
          diagnostics.push({
            code: protectedIds.has(operation.id) ? "VECTOR180-PATCH-REFERENCE" : "VECTOR180-PATCH-OVERLAP",
            severity: "error",
  ...
  function decodePatch(input) {
    ...
    if (typeof input.baseSha256 !== "string" || !SHA256_PATTERN2.test(input.baseSha256)) {
      diagnostics.push({
        code: "VECTOR180-PATCH-SCHEMA",
        severity: "error",
    ...
    if (operations.filter((operation) => operation.op === "clone-connector").length > 1) {
      diagnostics.push({
        code: "VECTOR180-PATCH-SCHEMA",
        severity: "error",
        message: "vector180-patch/0.1 permits at most one clone-connector operation."
      });
    }
    if (hasErrors(diagnostics) || typeof input.baseSha256 !== "string" || input.schema !== "vector180-patch/0.1") {
      return { diagnostics };
    }
    const metadata = {
  ...
  function decodeOperation(input, index, diagnostics) {
    if (!isRecord3(input) || typeof input.op !== "string") {
      diagnostics.push(
    ...
    if (input.op === "set-text") {
      ...
      if (typeof input.id !== "string" || typeof input.oldText !== "string" || typeof input.value !== "string") {
        diagnostics.push(
          schemaOperationDiagnostic(
            index,
  ...
  function unsupportedAtomOperation(operationIndex, operation) {
    return {
      code: "VECTOR180-PATCH-UNSUPPORTED",
      severity: "error",
  ...
  // src/ops/metadata.ts
  var MAX_TEMPLATE_BASIS_BYTES = 8 * 1024 * 1024;
  async function projectAtomMetadata(atom) {
    const resolved = resolveVector180Atom(atom);
    const stylePaletteSha256 = resolved.model === void 0 ? void 0 : resolved.model.stylePaletteSha256;
    ...
    return Object.freeze({
      ...
      atomId: atom.id,
      sourceSha256: atom.source.sha256,
      metadataStatus: hasInvalidMetadata ? "invalid" : atom.metadata === void 0 ? "absent" : "present",
      ...atom.metadata === void 0 ? {} : { metadata: atom.metadata },
      ...atom.metadataSha256 === void 0 ? {} : { metadataSha256: atom.metadataSha256 },
      templateLineageStatus: atom.metadata?.templateLineage === void 0 ? "absent" : "asserted",
      ...stylePaletteSha256 === void 0 ? {} : { stylePaletteSha256 },
      diagnostics: [...resolved.diagnostics]
    });
  }
  async function compareAtomMetadata(leftAtom, rightAtom, options = {}) {
    const templateBasisTooLarge = options.templateBasisBytes !== void 0 && options.templateBasisBytes.byteLength > MAX_TEMPLATE_BASIS_BYTES;
    const templateBasisBytes = options.templateBasisBytes === void 0 || templateBasisTooLarge ? void 0 : options.templateBasisBytes.slice();
    ...
    let classification;
    if (left.metadataStatus === "invalid" || right.metadataStatus === "invalid" || left.stylePaletteSha256 === void 0 || right.stylePaletteSha256 === void 0) {
      classification = "insufficient-evidence";
    } else if (left.metadata?.templateLineage !== void 0 && right.metadata?.templateLineage !== void 0) {
      ...
      if (!sameDeclaration) {
        ...
        classification = "insufficient-evidence";
      } else if (templateBasisBytes === void 0) {
        classification = "matching-asserted-template";
      } else {
        const basisSha256 = await sha256Hex(templateBasisBytes);
        if (basisSha256 === left.metadata.templateLineage.templateSha256 && basisSha256 === right.metadata.templateLineage.templateSha256) {
          ...
          diagnostics.push({
            code: "VECTOR180-METADATA-VERIFICATION",
            severity: "error",
            message: "Supplied template-basis bytes do not match both declared templateSha256 values."
          });
        }
      }
    } else if (left.metadata?.styleFamily !== void 0 && right.metadata?.styleFamily !== void 0) {
      classification = sameJson(
        left.metadata.styleFamily,
        right.metadata.styleFamily
      ) ? "matching-declared-style-family" : "different";
    } else {
      classification = left.stylePaletteSha256 === right.stylePaletteSha256 ? "matching-derived-style-palette" : "different";
    }
    return Object.freeze({
  ...
  async function deriveStylePaletteSha2562(objects) {
    const tuples = /* @__PURE__ */ new Set();
    visitResolvedObjects3(objects, (object) => {
      if (object.kind === "group") return;
      tuples.add(
        JSON.stringify([
          object.kind,
          object.style.fill,
          object.style.stroke,
          canonicalNumber2(object.style.strokeWidth),
          canonicalNumber2(object.style.opacity),
          object.style.fontFamily ?? null,
          object.style.fontSize === void 0 ? null : canonicalNumber2(object.style.fontSize),
          object.style.fontFamily === void 0 ? null : object.style.fontWeight,
          object.style.fontFamily === void 0 ? null : object.style.fontStyle
        ])
      );
  ...
  function visitResolvedObjects3(objects, visitor) {
    for (const object of objects) {
      visitor(object);
      if (object.kind === "group") {
        visitResolvedObjects3(object.children, visitor);
      }
    }
  }
  ...
  function diffVector180Atoms(leftAtom, rightAtom) {
    ...
    const diagnostics = [];
    if (leftResolved.model === void 0 || hasErrors(leftResolved.diagnostics)) {
      diagnostics.push({
        code: "VECTOR180-DIFF-INVALID-LEFT",
        severity: "error",
        message: "The left atom does not independently pass complete C4/C6 resolution."
      });
      diagnostics.push(...leftResolved.diagnostics);
    }
    if (rightResolved.model === void 0 || hasErrors(rightResolved.diagnostics)) {
      diagnostics.push({
        code: "VECTOR180-DIFF-INVALID-RIGHT",
        severity: "error",
        message: "The right atom does not independently pass complete C4/C6 resolution."
      });
      diagnostics.push(...rightResolved.diagnostics);
    }
    if (leftResolved.model === void 0 || rightResolved.model === void 0) {
      return incomparable(
        leftIdentity,
        rightIdentity,
        lexical,
    ...
    const leftNodes = indexSourceNodes2(leftAtom.children);
    const rightNodes = indexSourceNodes2(rightAtom.children);
    const leftObjects = indexResolvedObjects2(leftResolved.model.objects);
    const rightObjects = indexResolvedObjects2(rightResolved.model.objects);
    if (leftNodes.size !== leftObjects.size || rightNodes.size !== rightObjects.size) {
      diagnostics.push({
        code: "VECTOR180-DIFF-IDENTITY",
        severity: "error",
        message: "C4 and C6 stable-ID inventories disagree; semantic comparison is ambiguous."
      });
      return incomparable(
        leftIdentity,
        rightIdentity,
    ...
    if (!sameJson2(leftAtom.viewBox, rightAtom.viewBox)) {
      changes.push(
        rootChange(
          leftAtom,
          rightAtom,
    ...
    const allIds = /* @__PURE__ */ new Set([...leftNodes.keys(), ...rightNodes.keys()]);
    ...
    for (const id of allIds) {
      ...
      if (leftNode === void 0 || leftObject === void 0) {
        if (rightNode !== void 0 && rightObject !== void 0) {
          changes.push({
            id,
            kind: "added",
            fieldPath: "/objects",
    ...
    for (const section of metadata.changedSections) {
      changes.push({
        ...
        ...leftAtom.metadata?.[section] === void 0 ? {} : {
          left: rootSide(leftAtom, asJsonValue(leftAtom.metadata[section]))
        },
        ...rightAtom.metadata?.[section] === void 0 ? {} : {
          right: rootSide(
            rightAtom,
    ...
    changes.sort((left, right) => {
      ...
      return leftTraversal - rightTraversal || left.id.localeCompare(right.id) || CHANGE_ORDER[left.kind] - CHANGE_ORDER[right.kind] || left.fieldPath.localeCompare(right.fieldPath);
    });
    ...
    const semanticChanges = changes.filter(
      (change) => change.kind !== "metadata" && !(change.kind === "root" && change.fieldPath === "/wireFamily")
    );
    const classification = lexical.equal ? "exact" : semanticChanges.length === 0 ? "semantic-equivalent" : "changed";
    ...
    if (new TextEncoder().encode(canonicalJsonText(report)).byteLength > MAX_REPORT_BYTES) {
      diagnostics.push({
        code: "VECTOR180-DIFF-LIMIT",
        severity: "error",
  ...
  function compareMetadata(left, right) {
    const changedSections = ["hydration", "templateLineage", "styleFamily"].filter(
      (section) => !sameJson2(left.metadata?.[section], right.metadata?.[section])
    );
    return {
      classification: left.metadata === void 0 && right.metadata === void 0 ? "absent" : changedSections.length === 0 ? "equal" : "changed",
      ...left.metadataSha256 === void 0 ? {} : { leftSha256: left.metadataSha256 },
      ...right.metadataSha256 === void 0 ? {} : { rightSha256: right.metadataSha256 },
      changedSections
    };
  }
  function indexSourceNodes2(children) {
    const result = /* @__PURE__ */ new Map();
    let traversal = 1;
    const visit2 = (nodes, parentId) => {
      nodes.forEach((node, order) => {
        result.set(node.id, { node, parentId, order, traversal });
        traversal += 1;
        visit2(node.children, node.id);
      });
    };
    visit2(children, null);
    return result;
  }
  function indexResolvedObjects2(objects) {
    const result = /* @__PURE__ */ new Map();
    let traversal = 1;
    const visit2 = (values) => {
      for (const object of values) {
        ...
        traversal += 1;
        if (object.kind === "group") visit2(object.children);
      }
    };
    visit2(objects);
    return result;
  }
  function compareField(changes, id, kind, fieldPath, leftValue, rightValue, leftNode, rightNode) {
    if (sameJson2(leftValue, rightValue)) return;
    changes.push({
      id,
      kind,
      fieldPath,
  ...
  function side(indexed, snapshot2) {
    return {
      parentId: indexed.parentId,
      order: indexed.order,
      sourceRange: indexed.node.sourceRange,
      snapshot: normalizeJson(snapshot2)
    };
  }
  function rootSide(atom, snapshot2) {
    return {
      sourceRange: atom.sourceRange,
      snapshot: normalizeJson(snapshot2)
    };
  }
  function rootChange(left, right, fieldPath, leftValue, rightValue) {
    return {
      id: left.id,
      kind: "root",
      fieldPath,
  ...
  function relationshipSnapshot(object) {
    return object.kind === "line" ? {
      fromId: object.fromId ?? null,
      toId: object.toId ?? null
    } : null;
  }
  function textSnapshot(object) {
    return object.kind === "text" ? object.lines.map((line) => ({ text: line.text, x: line.x, y: line.y })) : null;
  }
  ...
  function transformSnapshot(object) {
    return object.kind === "group" ? { x: object.translateX, y: object.translateY } : null;
  }
  function frameSnapshot(object) {
  ...
  function completeSnapshot(node, object) {
    return {
      ...
      ...object.kind === "group" ? {
        children: object.children.map(
          (child) => completeSnapshot(
            node.children.find((candidate) => candidate.id === child.id) ?? node,
            child
          )
        )
      } : {}
  ...
  function summarize(changes, unchangedObjects, changedObjects) {
  ...
  function normalizeJson(value) {
    ...
    return Object.fromEntries(
      Object.entries(value).sort(([left], [right]) => left.localeCompare(right)).map(([key, child]) => [key, normalizeJson(child)])
    );
  }

  // src/ops/migrate.ts
  var CONTROL_PREFIX = "data-pptv-";
  var CANONICAL_CONTROL_PREFIX = "data-vector180-";
  var DISCOVERY_REPLACEMENTS = [
    [
  ...
  var VECTOR180_ATOM_DISCOVERY_COMMENT2 = [
    "<!--",
    "Vector180 atom: deterministic, editable vector source.",
    "Authoring skill: vector180-authoring",
    "https://github.com/willackerly/office180-md-office-converter/tree/main/.agents/skills/vector180-authoring",
  ...
  async function migratePptvAtom(input) {
    let loaded;
    try {
        ...
        error instanceof Vector180LoadError ? error.diagnostics : [
          {
            ...
            severity: "error",
            message: error instanceof Error ? error.message : "Legacy atom could not be loaded."
          }
        ]
      );
    }
      ...
      return refused([
        {
          ...
          severity: "error",
          message: "Direct migration accepts a legacy standalone PPTV SVG atom only; hydrate a legacy HTML deck slide with extract."
        }
      ]);
    }
    const legacy = loaded;
      ...
      return refused([
        {
          code: "VECTOR180-MIGRATION-REFUSED",
          severity: "error",
          message: "migrate accepts one independently valid legacy PPTV standalone SVG atom only."
        }
      ]);
    }
    const legacyResolved = resolveVector180Atom(legacy);
    if (legacyResolved.model === void 0) {
      return refused([
        {
          code: "VECTOR180-MIGRATION-REFUSED",
          severity: "error",
      ...
      return refused([
        {
          code: "VECTOR180-MIGRATION-REFUSED",
          severity: "error",
          message: "Legacy atom contains no migratable reserved control spelling."
        }
      ]);
    }
    ...
    try {
      canonical = await loadAtom({
        kind: "text",
        text: sourceText,
        ...legacy.source.name === void 0 ? {} : { name: canonicalName(legacy.source.name) }
      });
    } catch (error) {
      return refused([
        {
    ...
    if (canonical.wireFamily !== "vector180" || resolveVector180Atom(canonical).model === void 0) {
      return refused([
        {
          code: "VECTOR180-MIGRATION-REFUSED",
          severity: "error",
          message: "Migrated candidate failed independent canonical C6 resolution."
        },
      ...
      return refused([
        {
          code: "VECTOR180-MIGRATION-REFUSED",
          severity: "error",
          message: "Namespace migration did not prove complete C12 semantic equivalence."
        },
    ...
    const report = Object.freeze({
      ...
      metadataDisposition: containsOpaqueMetadata(legacy.source.text) ? "preserved-opaque" : "absent",
      semanticComparison: {
        schema: "vector180-source-diff/0.1",
        reportSha256,
  ...
  function planReplacements(atom) {
    ...
    const hasDiscoveryComment = atom.source.text.includes("Authoring skill: pptv-authoring") || atom.source.text.includes("Authoring skill: vector180-authoring");
    if (!hasDiscoveryComment) {
      ...
      replacements.push({
        ...
        replacement: insertionPoint === 0 ? `${VECTOR180_ATOM_DISCOVERY_COMMENT2}
` : `
${VECTOR180_ATOM_DISCOVERY_COMMENT2}`
      });
    }
  ...
  function applyReplacements(source, replacements) {
    ...
    for (const replacement of [...replacements].reverse()) {
      candidate = candidate.slice(0, replacement.charStart) + replacement.replacement + candidate.slice(replacement.charEnd);
    }
    return candidate;
  }
  function buildChangedRanges(sourceText, outputText, replacements) {
    ...
    return replacements.map((replacement) => {
      ...
      delta += replacement.replacement.length - (replacement.charEnd - replacement.charStart);
      return {
  ...
  function xmlDeclarationEnd(sourceText) {
  ...
  function canonicalName(name) {
    return name.toLowerCase().endsWith(".pptv.svg") ? `${name.slice(0, -".pptv.svg".length)}.vector180.svg` : name;
  }
  ...
  function outlineManifest(manifest, wireFamily) {
    return {
  ...
  function inventoryAtom(diagram) {
    return {
  ...
  function getSlide(deck, id, view = "semantic") {
    ...
    return {
      ...
      objects: slide.children.map(
        (node) => projectObject(node, view, deck.wireFamily)
      )
    };
  }
  function getAtom(diagram, view = "semantic") {
    return {
      schema: "vector180-atom/0.1",
      wireFamily: diagram.wireFamily,
      atomId: diagram.id,
      viewBox: [...diagram.viewBox],
      objects: diagram.children.map(
        (node) => projectObject(node, view, diagram.wireFamily)
      )
    };
  }
  function getObject(deck, id, view = "semantic") {
    ...
    for (const slideIdValue of deck.slideOrder) {
      ...
      if (found !== void 0) return projectObject(found, view, deck.wireFamily);
    }
    return void 0;
  }
  function getAtomObject(diagram, id, view = "semantic") {
    if (ambiguousAtomObjectIds(diagram).has(id)) return void 0;
  ...
  function queryAtomObjects(diagram, query, view = "semantic") {
    const result = [];
    const idSet = query.ids === void 0 ? void 0 : new Set(query.ids);
  ...
  function ambiguousObjectIds(deck) {
    return new Set(
      deck.diagnostics.flatMap(
        (diagnostic) => diagnostic.code === "VECTOR180-ID-DUPLICATE" && diagnostic.objectId !== void 0 ? [diagnostic.objectId] : []
      )
    );
  }
  function ambiguousAtomObjectIds(diagram) {
    return new Set(
  ...
  function extractAtomText(diagram) {
    const entries = [];
    visitNodes(diagram.children, (node) => {
      if (node.role === "text" && node.text !== void 0) {
        entries.push({
  ...
  function projectObject(node, view, wireFamily) {
    return {
      wireFamily,
      id: node.id,
      role: node.role,
      ...
      children: node.children.map(
        (child) => projectObject(child, view, wireFamily)
      ),
      ...view === "editing" ? {
  ...
  function inventoryAtomObject(node) {
    return {
      id: node.id,
      role: node.role,
      ...node.text === void 0 ? {} : { text: node.text },
  ...
  async function inspectVector180Conformance(input) {
    ...
    try {
      ...
      c4Diagnostics = error instanceof Vector180LoadError ? error.diagnostics : [runtimeDiagnostic("VECTOR180-BROWSER-C4", error)];
    }
    let resolved;
    let c6Diagnostics = [];
    if (document !== void 0) {
  ...
  function normalizeDocument(document) {
    return document.sourceKind === "html" ? normalizeDeck(document) : normalizeAtom(document);
  }
  ...
  function normalizeAtom(atom) {
    return toJson({
      ...
      index: {
        ...
        objects: [...atom.index.objects.entries()].sort(([left], [right]) => left.localeCompare(right)).map(([id, value]) => ({
          id,
          value: {
            ...value,
            attributeRanges: normalizeRangeMap(value.attributeRanges)
  ...
  function copyDiagnostic(diagnostic) {
    return {
      ...
      ...diagnostic.atomId === void 0 ? {} : { atomId: diagnostic.atomId },
  ...
  var EditorSession = class _EditorSession {
    ...
    static async open(input, options = {}) {
      return new _EditorSession(await loadVector180Document(input), options);
    }
    get state() {
  ...
  function intentToOperation(document, intent) {
    if (intent.kind === "set-active-theme") {
      if (document.sourceKind !== "html" || document.activeTheme === void 0) {
        return {
          diagnostics: [
            {
  ...
  async function prepareVector180BrowserTextMeasurer(sources, options = {}) {
  ...
  function validateRequest(request) {
    ...
    const hasAtom = "atomId" in request && typeof request.atomId === "string" && request.atomId.length > 0;
    if (hasSlide === hasAtom) {
      throw new Error(
