...
import type {
  Vector180PptxMap,
  Vector180PptxMapObject,
  Vector180PptxMapSlide,
...
export interface PptxObjectNormalizationEvidence {
  readonly ruleId: "vector180-c10/end-paragraph-style-marker-omitted/1";
  readonly partName: string;
...
export interface PptxInspection {
  readonly schema: "vector180-pptx-inspection/0.1";
  ...
  try {
    ...
    return {
      inspection: Object.freeze({
        schema: "vector180-pptx-inspection/0.1",
...
function inspectCustomProperties(
  ...
  for (const property of directElements(root, CUSTOM_PROPERTIES, "property")) {
    ...
    if (Object.hasOwn(result, name)) {
      diagnostics.push({
        code: "VECTOR180-RECONCILE-LINEAGE",
        severity: "error",
...
function validateLineage(
  ...
  const expected = {
    "vector180.activeTheme": "diagram-local",
    "vector180.compiler": map.compiler,
    "vector180.mapSchema": map.schema,
    "vector180.placement": JSON.stringify([
      placement.slideId,
      placement.policy,
      placement.x,
      placement.y,
    ...
    "vector180.resolvedSchema": map.resolvedSchema,
    "vector180.atomSha256": map.source.sha256,
    "vector180.sourceId": map.source.id,
    "vector180.sourceKind": map.source.kind,
    "vector180.sourceResolvedSchema": map.sourceResolvedSchema,
    "vector180.sourceSha256": map.composition.composedDeckSha256,
  };
...
function inspectSlideBindings(
  ...
  for (const [order, slideId] of slideIds.entries()) {
    ...
    if (expected === undefined || partName !== expected.partName) {
      diagnostics.push({
        code: "VECTOR180-RECONCILE-AMBIGUOUS",
        severity: "error",
...
function inspectSlide(
  root: XmlElement,
  mapSlide: Vector180PptxMapSlide,
  order: number,
  ...
  for (const object of discovered) {
    if (object.id === undefined) {
      diagnostics.push({
        code: "VECTOR180-RECONCILE-MISSING-ID",
        severity: "error",
  ...
  for (const [id, objects] of occurrences) {
    if (objects.length > 1) {
      diagnostics.push({
        code: "VECTOR180-RECONCILE-DUPLICATE-ID",
        severity: "error",
...
function inspectIdentityOccurrence(
  ...
  expected: Vector180PptxMapObject,
        ...
        normalizations: Object.freeze(
          text.endMarkerOmitted
            ? [
                {
                  ruleId:
                    "vector180-c10/end-paragraph-style-marker-omitted/1" as const,
...
function mapStyle(expected: Vector180PptxMapObject): PptxInspectedStyle {
...
function normalizeXml(
  ...
  if (maskedElement !== undefined) {
    return ["vector180-semantic", maskedElement];
  }
  ...
  if (maskedAttributes !== undefined) {
    attributes = {
      ...attributes,
      ...Object.fromEntries(
        [...maskedAttributes].map((name) => [name, "<vector180-value>"]),
      ),
    };
  }
...
function invalidPptxDiagnostic(message: string): Diagnostic {
  return {
    code: "VECTOR180-RECONCILE-INVALID-PPTX",
    severity: "error",
    message,
  };
}
