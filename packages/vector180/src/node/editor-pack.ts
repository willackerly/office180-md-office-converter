...
import { loadVector180Document, Vector180LoadError } from "../core/deck.js";
import {
...
const APP_URL = new URL(
  "../../assets/vector180-editor-0.1.app.js",
  import.meta.url,
);
const CSS_URL = new URL(
  "../../assets/vector180-editor-0.1.css",
  import.meta.url,
);
...
interface EditorPackPayloadBase {
  ...
  readonly fontEnvironment?: Vector180DefaultFontEnvironmentEvidence;
  readonly nearLimit: number;
}
...
interface AtomEditorPackPayload extends EditorPackPayloadBase {
...
type EditorPackPayload = DeckEditorPackPayload | AtomEditorPackPayload;
...
export async function createEditorPack(
  ...
  let document: Vector180Document;
  try {
    document = await loadVector180Document(input);
  } catch (error) {
  ...
  const fontMeasurer =
    options.fontFaces === undefined
      ...
      : await createFontkitTextMeasurer(options.fontFaces);
  ...
  if (document.sourceKind === "html") {
    const resolved = resolveVector180Deck(document);
    if (resolved.model === undefined) {
      return { diagnostics: [...resolved.diagnostics] };
    }
    const nodeTextFit = preflightDeckTextFit(resolved.model, fontMeasurer, {
      nearLimit,
    });
    const fonts = await embedExactFonts(fontMeasurer, nodeTextFit);
    payload = {
      schema: "vector180-editor-pack/0.1",
      documentKind: "deck",
      ...
      fonts,
      ...(fontMeasurer.defaultEnvironment === undefined
        ? {}
        : { fontEnvironment: fontMeasurer.defaultEnvironment }),
      nearLimit,
      nodeTextFit,
    };
  } else {
    const resolved = resolveVector180Atom(document);
    if (resolved.model === undefined) {
      return { diagnostics: [...resolved.diagnostics] };
    }
    const nodeTextFit = preflightAtomTextFit(
      resolved.model,
    ...
    payload = {
      ...
      documentKind: "atom",
      title: document.id,
      downloadName: canonicalDownloadName(document.source.name, "atom"),
      downloadMime: "image/svg+xml;charset=utf-8",
      sourceSha256: document.source.sha256,
      sourceBase64: Buffer.from(document.source.bytes).toString("base64"),
      outline: outlineAtom(document),
      inventory: inventoryAtom(document),
      resolved: resolved.model,
      diagnostics: resolved.diagnostics,
      fonts,
      ...(fontMeasurer.defaultEnvironment === undefined
...
async function embedExactFonts(
  ...
  textFit: Vector180DeckTextFitResult | Vector180AtomTextFitResult,
...
<head>
  ...
  <meta name="vector180-editor-runtime" content="vector180-editor/0.1">
  <link rel="icon" href="data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22/%3E">
  <title>Vector180 Editor</title>
  <style>${css}</style>
</head>
<body>
  <div class="app">
    <header class="toolbar">
      <div class="toolbar__title">
        <strong data-title>Vector180 Editor</strong>
    ...
    <main class="workspace">
      ...
      <section class="center" aria-label="Vector180 workspace">
      ...
      <aside class="panel" aria-label="Object inspector">
        ...
        <div class="text-editor">
          <label for="vector180-direct-text">Selected hard line</label>
          <input id="vector180-direct-text" type="text" data-text-edit disabled>
  ...
  <script id="vector180-editor-payload" type="application/json">${serializedPayload}</script>
  <script>${app}</script>
</body>
</html>
`;
...
function canonicalDownloadName(
  ...
  const suffix = kind === "deck" ? ".vector180.html" : ".vector180.svg";
  if (name === undefined || name.length === 0) {
    return kind === "deck" ? "deck.vector180.html" : "atom.vector180.svg";
  }
