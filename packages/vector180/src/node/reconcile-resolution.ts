...
const FINDING_ID_PATTERN = /^finding\.[0-9a-f]{16}$/u;
...
export interface Vector180ConnectorOccurrenceFingerprint {
...
export interface Vector180ReconcileResolution {
  readonly schema: "vector180-reconcile-resolution/0.1";
  readonly reconciliationReportSha256: string;
  readonly findingId: string;
  readonly classification: "one-baseline-equivalent-copy";
  ...
  readonly connector: Vector180ConnectorCloneState;
}

export class Vector180ReconcileResolutionError extends Error {
  readonly code = "VECTOR180-RECONCILE-RESOLUTION";

  constructor(message: string) {
    super(message);
    this.name = "Vector180ReconcileResolutionError";
  }
}

/**
 * Compute the exact proof digest a reviewed resolution must bind.
 *
 * Call this only on the unresolved reconciliation result returned without a
 * resolution. Canonical JSON is duplicate-free, recursively key-sorted, and
 * compact before the UTF-8 SHA-256 is taken.
 */
export function reconciliationReportSha256(report: unknown): string {
  return createHash("sha256").update(canonicalJsonText(report)).digest("hex");
}

export type Vector180ConnectorDuplicateResolutionStatus =
  | "eligible"
  | "no-baseline-match"
...
export interface Vector180ConnectorDuplicateResolutionAssessment {
  readonly status: Vector180ConnectorDuplicateResolutionStatus;
  ...
  readonly resolutionSchema: "vector180-reconcile-resolution/0.1";
  readonly nextActionIds: readonly string[];
}

export interface Vector180ConnectorDuplicateResolutionGuidance {
  readonly assessment: Vector180ConnectorDuplicateResolutionAssessment;
...
): Vector180ConnectorDuplicateResolutionGuidance {
      ...
      [
        {
          ...
          description:
            "Represent the intended connector inventory explicitly in canonical Vector180 source with fresh stable IDs and explicit bindings.",
          consequence:
    ...
    return guidance(
      ...
      [
        ...
        },
        {
          id: "author-connectors-in-source",
          description:
            "Represent the intended connector inventory explicitly in canonical Vector180 source.",
          consequence:
    ...
    return guidance(
      ...
      [
        ...
        },
        {
          id: "author-connectors-in-source",
          description:
            "Author the intended original and additional connector explicitly in canonical Vector180 source.",
          consequence:
    ...
    return guidance(
      ...
      [
        ...
        },
        {
          id: "author-connectors-in-source",
          description:
            "Author the additional connector explicitly in canonical Vector180 source with a fresh stable ID.",
          consequence:
...
export function parseVector180ReconcileResolution(
  input: unknown,
): Vector180ReconcileResolution {
  const value = requireRecord(input, "Resolution");
  requireExactKeys(value, RESOLUTION_KEYS, "Resolution");
  if (value["schema"] !== "vector180-reconcile-resolution/0.1") {
    fail(
      'Resolution schema must be exactly "vector180-reconcile-resolution/0.1".',
    );
  }
  const connector = parseConnector(value["connector"]);
  const result: Vector180ReconcileResolution = {
    schema: "vector180-reconcile-resolution/0.1",
    reconciliationReportSha256: requireSha256(
      value["reconciliationReportSha256"],
      "reconciliationReportSha256",
    ),
    findingId: requireFindingId(value["findingId"]),
...
function requireFindingId(value: unknown): string {
  if (typeof value !== "string" || !FINDING_ID_PATTERN.test(value)) {
    fail('findingId must match "finding.<16 lowercase hex characters>".');
  }
  return value;
}

function requireClassification(value: unknown): "one-baseline-equivalent-copy" {
  if (value !== "one-baseline-equivalent-copy") {
    fail('classification must equal "one-baseline-equivalent-copy".');
  }
  return value;
}
...
function guidance(
  assessment: Omit<
    Vector180ConnectorDuplicateResolutionAssessment,
    | "requiredEditedOccurrenceCount"
    | "requiredBaselineEquivalentOccurrenceCount"
    | "resolutionSchema"
  >,
  summary: string,
  options: Vector180ConnectorDuplicateResolutionGuidance["options"],
): Vector180ConnectorDuplicateResolutionGuidance {
  return deepFreeze({
    assessment: {
      ...assessment,
      ...
      resolutionSchema: "vector180-reconcile-resolution/0.1",
    },
    summary,
...
export function serializeVector180ReconcileResolution(input: unknown): string {
  return `${JSON.stringify(parseVector180ReconcileResolution(input), null, 2)}\n`;
}

export function fingerprintConnectorOccurrence(
  occurrence: PptxIdentityOccurrence,
): Vector180ConnectorOccurrenceFingerprint | undefined {
  if (
...
function parseConnector(value: unknown): Vector180ConnectorCloneState {
...
function parseEndpoints(value: unknown): Vector180ConnectorEndpoints {
...
function parseStyle(value: unknown): Vector180ConcreteNativeStyle {
...
function fail(message: string): never {
  throw new Vector180ReconcileResolutionError(message);
}
