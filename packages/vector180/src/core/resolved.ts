/**
 * Pure compiler-grade Vector180 geometry, text, and object projection.
 *
...
import { wireAttribute } from "./dialect.js";
import { canonicalJsonText } from "./metadata.js";
import { sha256HexSync } from "./source.js";
import {
...
const SVG_ASSET_ATTRIBUTES = boundaryAttributes(
  "data-vector180-bounds",
  "data-pptv-bounds",
);
...
export interface Vector180Bounds {
  readonly x: number;
  readonly y: number;
...
export interface Vector180Point {
  readonly x: number;
  readonly y: number;
}

export type Vector180ResolvedObjectKind =
  "rect" | "ellipse" | "text" | "line" | "group" | "svg-asset" | "raster-asset";

export interface Vector180ResolvedObjectBase {
  readonly id: string;
  readonly slideId: string;
  readonly parentId: string | null;
  readonly kind: Vector180ResolvedObjectKind;
...
export interface Vector180ResolvedRect extends Vector180ResolvedObjectBase {
  readonly kind: "rect";
  readonly x: number;
  readonly y: number;
  readonly width: number;
...
export interface Vector180ResolvedEllipse extends Vector180ResolvedObjectBase {
...
export interface Vector180ResolvedGroup extends Vector180ResolvedObjectBase {
  readonly kind: "group";
  readonly translateX: number;
  readonly translateY: number;
  readonly children: readonly Vector180ResolvedObject[];
}

export interface Vector180ResolvedSvgAsset extends Vector180ResolvedObjectBase {
  readonly kind: "svg-asset";
}

export interface Vector180ResolvedRasterAsset extends Vector180ResolvedObjectBase {
  readonly kind: "raster-asset";
  readonly resourceRef: string;
}

export interface Vector180ResolvedTextLine {
  readonly text: string;
  readonly x: number;
  readonly y: number;
}

export interface Vector180ResolvedText extends Vector180ResolvedObjectBase {
  readonly kind: "text";
  readonly frame: Vector180Bounds;
  readonly lineStep: number;
  readonly anchor: "start" | "middle" | "end";
  readonly lines: readonly Vector180ResolvedTextLine[];
  readonly wrap: "none";
...
export type Vector180ResolvedObject =
  | Vector180ResolvedRect
  | Vector180ResolvedEllipse
  | Vector180ResolvedText
  | Vector180ResolvedLine
  | Vector180ResolvedGroup
  | Vector180ResolvedSvgAsset
  | Vector180ResolvedRasterAsset;

export interface Vector180ResolvedSlide {
  readonly id: string;
  readonly order: number;
  readonly hidden: boolean;
  readonly layout?: string;
  readonly objects: readonly Vector180ResolvedObject[];
}

export interface Vector180ResolvedDeck {
...
export interface Vector180ResolvedDeckResult {
  readonly model?: Vector180ResolvedDeck;
  readonly diagnostics: readonly Diagnostic[];
}

export interface Vector180ResolvedAtomObjectBase extends Omit<
  Vector180ResolvedObjectBase,
  "slideId"
> {
  readonly atomId: string;
}

type AtomScoped<T> = Omit<T, "slideId"> & {
  readonly atomId: string;
};

export type Vector180ResolvedAtomRect = AtomScoped<Vector180ResolvedRect>;
export type Vector180ResolvedAtomEllipse = AtomScoped<Vector180ResolvedEllipse>;
export type Vector180ResolvedAtomText = AtomScoped<Vector180ResolvedText>;
export type Vector180ResolvedAtomLine = AtomScoped<Vector180ResolvedLine>;
export type Vector180ResolvedAtomSvgAsset =
  AtomScoped<Vector180ResolvedSvgAsset>;
export type Vector180ResolvedAtomRasterAsset =
  AtomScoped<Vector180ResolvedRasterAsset>;
export type Vector180ResolvedAtomGroup = AtomScoped<
  Omit<Vector180ResolvedGroup, "children">
> & {
  readonly children: readonly Vector180ResolvedAtomObject[];
};

export type Vector180ResolvedAtomObject =
  | Vector180ResolvedAtomRect
  | Vector180ResolvedAtomEllipse
  | Vector180ResolvedAtomText
  | Vector180ResolvedAtomLine
  | Vector180ResolvedAtomGroup
  | Vector180ResolvedAtomSvgAsset
  | Vector180ResolvedAtomRasterAsset;

export interface Vector180ResolvedAtom {
...
export interface Vector180ResolvedAtomResult {
  readonly model?: Vector180ResolvedAtom;
  readonly diagnostics: readonly Diagnostic[];
}

type Vector180ResolvableDocument = Vector180Deck | Vector180Atom;
...
type InternalResolvedObjectBase = InternalScoped<Vector180ResolvedObjectBase>;
type InternalResolvedRect = InternalScoped<Vector180ResolvedRect>;
type InternalResolvedEllipse = InternalScoped<Vector180ResolvedEllipse>;
type InternalResolvedText = InternalScoped<Vector180ResolvedText>;
type InternalResolvedLine = InternalScoped<Vector180ResolvedLine>;
type InternalResolvedSvgAsset = InternalScoped<Vector180ResolvedSvgAsset>;
type InternalResolvedRasterAsset = InternalScoped<Vector180ResolvedRasterAsset>;
...
interface ResolutionContext {
  readonly document: Vector180ResolvableDocument;
  readonly diagnostics: Diagnostic[];
  readonly styles: ReadonlyMap<string, Vector180ResolvedObjectStyle>;
  readonly sourceNodes: ReadonlyMap<string, Vector180Node>;
}
...
export function resolveVector180Deck(
  ...
  if (!validBaseSnapshot(deck)) {
    diagnostics.push({
      code: "VECTOR180-PROFILE-INVALID-BASE",
      severity: "error",
      message:
  ...
  const styleResult = resolveVector180Styles(deck);
  ...
  const slides: Vector180ResolvedSlide[] = [];
  ...
  for (const [order, slideId] of deck.slideOrder.entries()) {
    ...
    if (!exactCanvas(slide.viewBox)) {
      diagnostics.push({
        code: "VECTOR180-PROFILE-VIEWBOX",
        severity: "error",
  ...
  const model: Vector180ResolvedDeck = {
...
export function resolveVector180Atom(
  ...
  const diagnostics = [...atom.diagnostics];
  const styleResult = resolveVector180AtomStyles(atom);
  diagnostics.push(...styleResult.diagnostics);
  if (!validAtomBaseSnapshot(atom)) {
    diagnostics.push({
      code: "VECTOR180-PROFILE-INVALID-BASE",
      severity: "error",
      message:
        "Atom resolution requires a complete, unambiguous standalone semantic snapshot with no errors.",
      atomId: atom.id,
    });
    return { diagnostics };
  }
  if (!validAtomCanvas(atom.viewBox)) {
    diagnostics.push({
      code: "VECTOR180-PROFILE-VIEWBOX",
      severity: "error",
      message: `Atom "${atom.id}" requires a four-number finite viewBox with positive width and height.`,
      atomId: atom.id,
      range: atom.sourceRange,
    });
    return { diagnostics };
  }
  ...
  const context: ResolutionContext = {
    ...
    diagnostics,
    styles: styleResult.styles,
    sourceNodes: indexSourceNodes(atom),
  };
  const scope: ResolutionScope = {
    kind: "diagram",
    id: atom.id,
  ...
  const model: Vector180ResolvedAtom = {
    schema: "vector180-resolved-atom/0.1",
    sourceWireFamily: atom.wireFamily,
    sourceSha256: atom.source.sha256,
    atomId: atom.id,
    ...(atom.metadata === undefined ||
    atom.metadataSha256 === undefined ||
    atom.index.metadata === undefined
      ? {}
      : {
          metadata: {
            value: atom.metadata,
            metadataSha256: atom.metadataSha256,
            canonicalJson: canonicalJsonText(atom.metadata),
            sourceRange: atom.index.metadata.range,
            contentRange: atom.index.metadata.contentRange,
          },
        }),
    stylePaletteSha256: deriveStylePaletteSha256(
      internalObjects.map(toAtomObject),
    ),
    canvas: { viewBox: [...atom.viewBox] },
    objects: internalObjects.map(toAtomObject),
  };
  return { model: cloneAndFreeze(model), diagnostics };
}

function deriveStylePaletteSha256(
  objects: readonly Vector180ResolvedAtomObject[],
): string {
  const tuples = new Set<string>();
  visitResolvedAtomObjects(objects, (object) => {
  ...
  const canonicalPalette = `[${[...tuples].sort().join(",")}]`;
  return sha256HexSync(new TextEncoder().encode(canonicalPalette));
}

function visitResolvedAtomObjects(
  objects: readonly Vector180ResolvedAtomObject[],
  ...
  for (const object of objects) {
    ...
    if (object.kind === "group") {
      visitResolvedAtomObjects(object.children, visitor);
    }
  }
}
...
function validBaseSnapshot(deck: Vector180Deck): boolean {
  if (
...
function validAtomBaseSnapshot(diagram: Vector180Atom): boolean {
  if (
...
function indexSourceNodes(
  document: Vector180ResolvableDocument,
): Map<string, Vector180Node> {
  const nodes = new Map<string, Vector180Node>();
...
function toDeckObject(object: InternalResolvedObject): Vector180ResolvedObject {
  if (object.kind === "group") {
    ...
    } as Vector180ResolvedGroup;
  }
  const { containerId, ...fields } = object;
  return { ...fields, slideId: containerId } as Vector180ResolvedObject;
}

function toAtomObject(
  object: InternalResolvedObject,
): Vector180ResolvedAtomObject {
  if (object.kind === "group") {
    const { containerId, children, ...fields } = object;
    return {
      ...fields,
      atomId: containerId,
      children: children.map(toAtomObject),
    } as Vector180ResolvedAtomGroup;
  }
  const { containerId, ...fields } = object;
  return { ...fields, atomId: containerId } as Vector180ResolvedAtomObject;
}
...
function resolveSiblings(
  ...
  parentWorldOffset: Vector180Point,
...
function resolveRect(
  ...
  parentId: string | null,
  order: number,
  worldOffset: Vector180Point,
  styleEntry: Vector180ResolvedObjectStyle,
  context: ResolutionContext,
...
function resolveLine(
  ...
  const worldBounds = offsetBounds(localBounds, worldOffset, node, context);
  if (worldBounds === undefined) return undefined;
  const fromId = getAttribute(
    node,
    wireAttribute(context.document.wireFamily, "from"),
  );
  const toId = getAttribute(
    node,
    wireAttribute(context.document.wireFamily, "to"),
  );
  return {
    ...objectBase(
      node,
...
function resolveText(
  ...
  const frameAttribute = wireAttribute(context.document.wireFamily, "frame");
  const lineStepAttribute = wireAttribute(
    context.document.wireFamily,
    "line-step",
  );
  const frame = parseTupleBounds(
    getAttribute(node, frameAttribute),
    "VECTOR180-PROFILE-TEXT-FRAME",
    "text frame",
    node,
    context,
  );
  const lineStepSource = getAttribute(node, lineStepAttribute);
  ...
  if (lineStep === undefined) {
    pushNodeDiagnostic(
      ...
      node,
      "VECTOR180-PROFILE-TEXT-FRAME",
      `Text object "${node.id}" requires a positive finite ${lineStepAttribute}.`,
    );
  }
  if (
...
function parseHardLines(
  ...
  frame: Vector180Bounds,
  lineStep: number,
  lineStepSource: string,
  context: ResolutionContext,
): Vector180ResolvedTextLine[] | undefined {
  ...
  const lines: Vector180ResolvedTextLine[] = [];
  ...
  for (const child of elementChildren) {
    ...
    if (
      ...
      pushTextLinesDiagnostic(
        context,
        node,
        "successive tspan baselines must differ by exactly data-vector180-line-step",
      );
      return undefined;
    }
    lines.push({ text, x, y });
...
function objectBase<Kind extends Vector180ResolvedObjectKind>(
  node: Vector180Node,
  scope: ResolutionScope,
  parentId: string | null,
  kind: Kind,
  order: number,
  localBounds: Vector180Bounds,
  worldBounds: Vector180Bounds,
  worldOffset: Vector180Point,
  styleEntry: Vector180ResolvedObjectStyle,
...
function parseDeclaredBounds(
  node: Vector180Node,
  context: ResolutionContext,
): Vector180Bounds | undefined {
  return parseTupleBounds(
    getAttribute(node, wireAttribute(context.document.wireFamily, "bounds")),
...
function offsetBounds(
  bounds: Vector180Bounds,
  offset: Vector180Point,
  node: Vector180Node,
  context: ResolutionContext,
): Vector180Bounds | undefined {
  const result = {
...
function unionBounds(
  bounds: readonly Vector180Bounds[],
): Vector180Bounds | undefined {
...
function boundsAreFinite(bounds: Vector180Bounds): boolean {
  return (
...
function pointInside(bounds: Vector180Bounds, x: number, y: number): boolean {
  return (
    x >= bounds.x &&
...
function nodeSourceScope(
  ...
  return {
    atomId: context.document.index.objects.get(node.id)?.atomId ?? "",
  };
}
