/**
 * Pure non-mutating text-fit preflight for explicit Vector180 hard lines.
 *
...
export interface Vector180DeckTextMeasureRequest {
  readonly slideId: string;
  readonly objectId: string;
  readonly lineIndex: number;
  readonly text: string;
  readonly font: Vector180TextFont;
}

export interface Vector180AtomTextMeasureRequest {
...
export interface Vector180MeasuredText {
...
export interface Vector180UnverifiedText {
...
export type Vector180TextMeasurement =
  Vector180MeasuredText | Vector180UnverifiedText;

export type Vector180DeckTextMeasurer = (
  request: Vector180DeckTextMeasureRequest,
) => Vector180TextMeasurement;

export type Vector180AtomTextMeasurer = (
  request: Vector180AtomTextMeasureRequest,
) => Vector180TextMeasurement;

export interface Vector180TextFitOptions {
  /** Utilization at or above this value warns before an actual overrun. */
  readonly nearLimit?: number;
}

export interface Vector180DeckTextFitLine {
  readonly slideId: string;
  readonly objectId: string;
  readonly lineIndex: number;
  readonly text: string;
  ...
  readonly font: Vector180TextFont;
  readonly status: Vector180TextFitStatus;
...
export interface Vector180AtomTextFitLine {
  readonly atomId: string;
  readonly objectId: string;
  readonly lineIndex: number;
  readonly text: string;
...
export interface Vector180TextFitSummary {
...
export interface Vector180DeckTextFitResult {
  readonly schema: "vector180-text-fit-deck/0.1";
  readonly sourceSha256: string;
  readonly nearLimit: number;
  readonly summary: Vector180TextFitSummary;
  readonly lines: readonly Vector180DeckTextFitLine[];
}

export interface Vector180AtomTextFitResult {
  readonly schema: "vector180-text-fit-atom/0.1";
  readonly sourceSha256: string;
  readonly atomId: string;
  readonly nearLimit: number;
  readonly summary: Vector180TextFitSummary;
  readonly lines: readonly Vector180AtomTextFitLine[];
}
...
>;
type MeasureResolvedLine = (
  font: Vector180TextFont,
) => Vector180TextMeasurement;
...
export function preflightDeckTextFit(
  ...
  const lines: Vector180DeckTextFitLine[] = [];
...
export function preflightAtomTextFit(
  ...
  const lines: Vector180AtomTextFitLine[] = [];
  visitAtomTextObjects(diagram.objects, (object) => {
    for (const [lineIndex, line] of object.lines.entries()) {
      lines.push(
        preflightAtomLine(object, line, lineIndex, nearLimit, measurer),
      );
    }
  });
...
export function textLineAvailableWidth(
  object: Pick<Vector180ResolvedText, "anchor" | "frame">,
  line: Pick<Vector180ResolvedTextLine, "x">,
): number {
...
function preflightDeckLine(
  ...
  line: Vector180ResolvedTextLine,
  lineIndex: number,
  nearLimit: number,
  measurer: Vector180DeckTextMeasurer,
): Vector180DeckTextFitLine {
...
function preflightAtomLine(
  object: Vector180ResolvedAtomText,
  line: Vector180ResolvedTextLine,
  lineIndex: number,
  nearLimit: number,
  measurer: Vector180AtomTextMeasurer,
): Vector180AtomTextFitLine {
  const evidence = preflightLineEvidence(
    object,
    line,
    lineIndex,
  ...
  return Object.freeze({ atomId: object.atomId, ...evidence });
}
...
function preflightLineEvidence(
  ...
  const font: Vector180TextFont = Object.freeze({
    family,
    size,
    weight: object.style.fontWeight,
    style: object.style.fontStyle,
  });
  let normalized: Vector180TextMeasurement;
  try {
  ...
  const status: Vector180TextFitStatus =
    measuredWidth > availableWidth
      ? "overflow"
...
function normalizeMeasurement(
  value: Vector180TextMeasurement,
): Vector180TextMeasurement {
...
function invalidMeasurement(reason: string): Vector180UnverifiedText {
  return {
    kind: "unverified",
...
function visitTextObjects(
  ...
  visit: (object: Vector180ResolvedText) => void,
): void {
...
function visitAtomTextObjects(
  objects: readonly Vector180ResolvedAtomObject[],
  visit: (object: Vector180ResolvedAtomText) => void,
): void {
  for (const object of objects) {
    if (object.kind === "text") visit(object);
    else if (object.kind === "group")
      visitAtomTextObjects(object.children, visit);
  }
}

function summarizeLines(
  lines: readonly Pick<Vector180DeckTextFitLine, "status">[],
): Vector180TextFitSummary {
  return Object.freeze({
    total: lines.length,
