...
async function withTempDirectory<T>(
  ...
  const directory = await mkdtemp(join(tmpdir(), "vector180-atomic-io-"));
  try {
...
describe("atomic filesystem writes", () => {
  ...
  it("publishes exclusively without changing or leaking beside an existing destination", async () => {
    await withTempDirectory(async (directory) => {
      const path = join(directory, "diagram.vector180.svg");
      ...
      expect(await readdir(directory)).toEqual(["diagram.vector180.svg"]);
    });
  });
