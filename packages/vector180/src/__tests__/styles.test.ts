...
): Promise<Vector180StyleResolution> {
  const source = mutate(await readMinimalDeck());
  const deck = await loadDeck({
    kind: "text",
    text: source,
    name: "styles.vector180.html",
  });
  return resolveVector180Styles(deck);
}
...
describe("C6 constrained style resolution", () => {
    ...
    expect(result.styles.get("cover.title")).toEqual({
      ...
      styleProvenance: {
        fill: expect.objectContaining({
          origin: "base-rule",
          expression: "var(--vector180-text-primary)",
  ...
  it("validates every theme but substitutes only the selected theme", async () => {
    ...
    const incompleteInactive = await resolveSource((source) =>
      replaceRequired(source, "  --vector180-accent-1: #9a8cff;\n", ""),
    );
    ...
    const extraInactive = await resolveSource((source) =>
      replaceRequired(
        source,
        "  --vector180-font-major: ABeeZee;\n}\n</script>\n\n<script data-vector180-runtime",
        "  --vector180-font-major: ABeeZee;\n  --vector180-extra: #010203;\n}\n</script>\n\n<script data-vector180-runtime",
      ),
    );
  ...
  it("accepts one quoted concrete font and normalizes numeric px values", async () => {
    const result = await resolveSource((source) =>
      source.replaceAll(
        "--vector180-font-major: ABeeZee;",
        '--vector180-font-major: "Aptos Display";',
      ),
    );
    expect(result.diagnostics).toEqual([]);
    expect(result.styles.get("cover.title")?.style).toMatchObject({
  ...
  it.each([
    {
      ...
      code: "VECTOR180-PROFILE-CSS-SELECTOR",
    },
    {
      ...
      code: "VECTOR180-PROFILE-CSS-PROPERTY",
    },
    {
      name: "calc",
      mutate: (source: string) =>
      ...
      code: "VECTOR180-PROFILE-CSS-VALUE",
    },
    {
      name: "important",
      mutate: (source: string) =>
    ...
    },
    {
      ...
      mutate: (source: string) =>
        source.replaceAll(
          "--vector180-font-major: ABeeZee;",
          "--vector180-font-major: ABeeZee, sans-serif;",
        ),
    ...
    },
    {
      ...
      mutate: (source: string) =>
        replaceRequired(
          ...
          "--vector180-background: var(--vector180-text-primary);",
        ),
      code: "VECTOR180-PROFILE-CSS-VALUE",
    },
  ...
  it("rejects unsupported local presentation and inline properties", async () => {
    ...
    expect(
      presentation.diagnostics.find(
        (diagnostic) =>
          diagnostic.code === "VECTOR180-PROFILE-CSS-PROPERTY" &&
  ...
  it("is deterministic and omits ignored objects from its lookup", async () => {
    const mutate = (source: string): string =>
      replaceRequired(
        source,
        'id="architecture.edge.client-policy" class="diagram-edge"\n          data-vector180-role="connector" data-vector180-export="native"',
        'id="architecture.edge.client-policy" class="diagram-edge"\n          data-vector180-role="connector" data-vector180-export="ignore"',
      );
