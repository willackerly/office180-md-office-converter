...
describe("Vector180 atomic preserve patches", () => {
  ...
  it("rejects stale, failed-precondition, and non-permutation transactions", async () => {
    ...
    expect(errorCodes(stale.diagnostics)).toContain("VECTOR180-PATCH-STALE");
    expect(errorCodes(precondition.diagnostics)).toContain(
      "VECTOR180-PATCH-PRECONDITION",
    );
    expect(errorCodes(order.diagnostics)).toContain("VECTOR180-PATCH-SCHEMA");
  ...
  it("rejects operation identifiers outside the stable-ID grammar", async () => {
    ...
    expect(errorCodes(diagnostics)).toContain("VECTOR180-PATCH-SCHEMA");
  });

  it.each([
    {
      label: "set-text oldText",
      operation: { op: "set-text", id: "cover.title", value: "Changed" },
    },
    {
      label: "set-active-theme oldTheme",
      operation: { op: "set-active-theme", theme: "dark" },
    },
    {
      label: "set-slide-order oldOrder",
      operation: {
        op: "set-slide-order",
        order: ["architecture", "cover"],
      },
    },
  ])("requires the complete $label precondition", async ({ operation }) => {
    const deck = await loadDeck({
      kind: "text",
      text: await readMinimalDeck(),
    });
  ...
  it("aligns timestamp and unique-order checks with the published schema", async () => {
    ...
    expect(
      diagnostics.filter(
        (diagnostic) => diagnostic.code === "VECTOR180-PATCH-SCHEMA",
      ),
    ).toHaveLength(2);
  });
  ...
  it("rejects a mixed valid/invalid transaction without returning partial edits", async () => {
    ...
    expect(errorCodes(result.diagnostics)).toContain("VECTOR180-PATCH-TARGET");
  });
  ...
  it("detects intersecting operation ranges before editing", async () => {
    ...
    expect(errorCodes(diagnostics)).toContain("VECTOR180-PATCH-OVERLAP");
  });
  ...
  it("rejects set-text across an opaque text boundary", async () => {
    const source = (await readMinimalDeck()).replace(
      ...
      'id="cover.title" class="cover-title"\n          data-vector180-role="text" data-vector180-export="svg"',
    );
    const deck = await loadDeck({ kind: "text", text: source });
    const result = await applyPatch(deck, {
      schema: "vector180-patch/0.1",
  ...
  it("rejects two insertions at the same empty text range", async () => {
    ...
    expect(result.applied).toBe(false);
    expect(errorCodes(result.diagnostics)).toContain("VECTOR180-PATCH-OVERLAP");
  });
