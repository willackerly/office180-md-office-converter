    ...
    const capture: Capture = {
      ...
      rows: oracleRows.map((oracle) => ({
        ...
        fontIdentity: [
          ...
          `alias=Vector180_${inputIdentity.font.sha256.slice(0, 24)}_${fontManifest.font.weight}_${fontManifest.font.style}_0`,
...
async function makeTemporaryDirectory(): Promise<string> {
  const directory = await mkdtemp(
    join(tmpdir(), "vector180-browser-calibration-test-"),
  );
