...
// CONTRACT:C6-PPTV-RESOLVED.2.0, CONTRACT:C9-PPTV-PPTX-BASELINE.2.0,
...
import type { Vector180Atom } from "../core/types.js";
import {
  compilePptxBaseline,
...
const TYPED_PLACEMENT: Vector180Placement = {
  slideId: "typed-slide",
  x: 0,
  y: 0,
  width: 800,
...
function typedAtomSource(): string {
  return `<svg xmlns="http://www.w3.org/2000/svg" data-vector180-version="0.1"
  id="typed-diagram" viewBox="0 0 800 600">
  <rect id="typed.rect" data-vector180-role="shape" data-vector180-export="native"
    x="50" y="60" width="160" height="90"
    fill="#112233" stroke="#445566" stroke-width="2"/>
  <ellipse id="typed.ellipse" data-vector180-role="shape" data-vector180-export="native"
    cx="330" cy="110" rx="60" ry="35"
    fill="#abcdef" stroke="#334455" stroke-width="2"/>
  <line id="typed.connector" data-vector180-role="connector" data-vector180-export="native"
    data-vector180-from="typed.rect" data-vector180-to="typed.ellipse"
    x1="210" y1="105" x2="270" y2="110"
    fill="none" stroke="#556677" stroke-width="3"/>
  <text id="typed.text" data-vector180-role="text" data-vector180-export="native"
    data-vector180-frame="80 210 300 70" data-vector180-line-step="32"
  ...
  <g id="typed.group" data-vector180-role="group" data-vector180-export="native"
    transform="translate(15 20)">
    <rect id="typed.group.box" data-vector180-role="shape" data-vector180-export="native"
      x="430" y="330" width="180" height="100"
      fill="#eeeeee" stroke="#222222" stroke-width="2"/>
    <text id="typed.group.label" data-vector180-role="text" data-vector180-export="native"
      data-vector180-frame="450 350 140 50" data-vector180-line-step="24"
...
async function typedFixture(
  placement: Vector180Placement = TYPED_PLACEMENT,
): Promise<BaselineFixture> {
  const sourceText = typedAtomSource();
  const diagram = await loadAtom({
    kind: "text",
    text: sourceText,
    name: "typed-diagram.vector180.svg",
  });
  return {
    sourceText,
    diagram,
...
async function withTempDirectory<T>(
  ...
  const directory = await mkdtemp(join(tmpdir(), "vector180-reconcile-test-"));
  try {
...
describe("C10 bounded PPTX reconciliation", () => {
  ...
  it("proves the exact native-save envelope and emits a deterministic agent-grade report", async () => {
    ...
    expect([...rules]).toEqual(
      expect.arrayContaining([
        ...
        "vector180-c10/end-paragraph-style-marker-omitted/1",
      ]),
    );
  });
  ...
  it("authenticates an exact native baseline before comparing a later edit against it", async () => {
    ...
    expect(unauthenticated).toMatchObject({
      status: "refused",
      diagnostics: [{ code: "VECTOR180-RECONCILE-INVALID-BASELINE" }],
    });
  ...
  it("keeps near-match normalization candidates fail-closed", async () => {
    ...
    expect(codes(transform)).toContain("VECTOR180-RECONCILE-UNSUPPORTED");
    expect(
    ...
    expect(codes(marker)).toContain("VECTOR180-RECONCILE-UNSUPPORTED");
    ...
    expect(codes(tableStyles)).toContain("VECTOR180-RECONCILE-UNSUPPORTED");
    expect(
  ...
  it("turns one direct single-line a:t edit into one minimal C5 set-text operation", async () => {
    ...
    const regenerated = await compilePptxBaseline(applied.atom!, {
      placement: IDENTITY_PLACEMENT,
    });
  ...
  it("authenticates, applies, and regenerates text-only edits through a canonical uniform map", async () => {
    ...
    const nonCanonical = structuredClone(uniform.map) as Vector180PptxMap & {
  ...
  it("refuses stale source, tampered map, lineage, invalid packages, and unstable identities", async () => {
    ...
    const stale = await loadAtom({
      kind: "text",
      text: `${sourceText}\n`,
      name: "stale.vector180.svg",
    });
    expect(
      await reconcilePptx(stale, artifact.map, artifact.pptxBytes),
    ).toMatchObject({
      status: "refused",
      diagnostics: [{ code: "VECTOR180-RECONCILE-STALE-SOURCE" }],
    });
    ...
    expect(codes(lineageResult)).toContain("VECTOR180-RECONCILE-LINEAGE");
    ...
    expect(codes(invalid)).toContain("VECTOR180-RECONCILE-INVALID-PPTX");
    ...
    expect(codes(unsafe)).toContain("VECTOR180-RECONCILE-INVALID-PPTX");
    ...
    expect(codes(missing)).toContain("VECTOR180-RECONCILE-MISSING-ID");
    ...
    expect(codes(duplicate)).toContain("VECTOR180-RECONCILE-DUPLICATE-ID");
  ...
  it("reports unsupported run, line, and package changes without a patch", async () => {
    ...
    for (const testCase of cases) {
      ...
      expect(codes(result), `${testCase.name} should be explicit`).toContain(
        "VECTOR180-RECONCILE-UNSUPPORTED",
      );
  ...
  it("inverts uniform placement exactly for connector endpoint edits", async () => {
    const uniformPlacement: Vector180Placement = {
  ...
  it("round-trips explicit group translation and direct text frame/anchor shifts", async () => {
    ...
    expect(applied.applied).toBe(true);
    expect(
      await compilePptxBaseline(applied.atom!, {
        placement: TYPED_PLACEMENT,
      }),
    ).toBeDefined();
  });
  ...
  it("round-trips pure within-parent painter order", async () => {
    ...
    const applied = await applyPatch(diagram, result.patch);
    expect(applied.applied).toBe(true);
    expect(applied.atom?.children.map((object) => object.id)).toEqual([
      "typed.ellipse",
      "typed.rect",
      "typed.connector",
      "typed.text",
  ...
  it("round-trips safe subtree deletion with same-transaction connector accounting", async () => {
    ...
    const applied = await applyPatch(diagram, result.patch);
    expect(applied.applied).toBe(true);
    expect(applied.atom?.index.objects.has("typed.ellipse")).toBe(false);
    expect(applied.atom?.index.objects.has("typed.connector")).toBe(false);
  });
  ...
  it("keeps group scaling, reparenting, implicit transforms, and inline styles fail-closed", async () => {
    ...
    expect(codes(scaled)).toContain("VECTOR180-RECONCILE-UNSUPPORTED");
    ...
    expect(codes(reparent)).toContain("VECTOR180-RECONCILE-AMBIGUOUS");
  ...
  it("publishes explicit CLI outputs atomically and leaves every input immutable", async () => {
    await withTempDirectory(async (directory) => {
      ...
      const baselinePath = join(directory, "atom.vector180.map.json");
      ...
      expect(failed.stderr.join("")).toContain("VECTOR180-RECONCILE-EXISTS");
