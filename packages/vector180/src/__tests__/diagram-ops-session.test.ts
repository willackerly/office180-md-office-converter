// Tests: CONTRACT:C4-PPTV-SOURCE.2.0, CONTRACT:C5-PPTV-PATCH.2.0

import { readFile } from "node:fs/promises";

import { describe, expect, it } from "vitest";

import { EditorSession } from "../browser/session.js";
import { loadAtom } from "../core/deck.js";
import {
  addVector180AtomDiscoveryComment,
  VECTOR180_ATOM_DISCOVERY_COMMENT,
} from "../core/extract.js";
import { applyPatch, validatePatch } from "../ops/patch.js";
import {
  extractAtomText,
  getAtom,
  getAtomObject,
  inventoryAtom,
  outlineAtom,
  queryAtomObjects,
} from "../ops/projections.js";
import { errorCodes } from "./test-helpers.js";

async function readMinimalAtom(): Promise<string> {
  return readFile(
    new URL(
      "../../../../examples/minimal-diagram.vector180.svg",
      import.meta.url,
    ),
    "utf8",
  );
}

async function readLegacyAtom(): Promise<string> {
  return readFile(
    new URL("../../../../examples/minimal-diagram.pptv.svg", import.meta.url),
    "utf8",
  );
}

describe("standalone diagram projections", () => {
  it("returns diagram-specific JSON-safe envelopes without synthetic slides", async () => {
    const diagram = await loadAtom({
      kind: "text",
      text: await readMinimalAtom(),
      name: "minimal-diagram.vector180.svg",
    });

    expect(outlineAtom(diagram)).toEqual({
      schema: "vector180-atom-outline/0.1",
      wireFamily: "vector180",
      version: "0.1",
      atomId: "system-overview",
      viewBox: [-100, -50, 1200, 800],
    });
    expect(inventoryAtom(diagram).objects.map((object) => object.id)).toEqual([
      "system-overview.background",
      "system-overview.title",
      "system-overview.flow",
      "system-overview.client",
      "system-overview.service",
    ]);
    expect(inventoryAtom(diagram).wireFamily).toBe("vector180");
    expect(getAtom(diagram).schema).toBe("vector180-atom/0.1");
    expect(getAtom(diagram).wireFamily).toBe("vector180");
    expect(
      getAtomObject(diagram, "system-overview.service.label", "editing"),
    ).toMatchObject({
      schema: "vector180-atom-object/0.1",
      wireFamily: "vector180",
      atomId: "system-overview",
      object: {
        wireFamily: "vector180",
        id: "system-overview.service.label",
        role: "text",
        text: "Policy service",
        attributes: {
          x: "700",
          y: "326",
        },
      },
    });
    expect(
      queryAtomObjects(diagram, {
        descendantOf: "system-overview.service",
      }),
    ).toEqual({
      schema: "vector180-atom-query/0.1",
      wireFamily: "vector180",
      atomId: "system-overview",
      objects: [
        expect.objectContaining({ id: "system-overview.service.panel" }),
        expect.objectContaining({ id: "system-overview.service.label" }),
      ],
    });

    const text = extractAtomText(diagram);
    expect(text.schema).toBe("vector180-atom-text/0.1");
    expect(text.wireFamily).toBe("vector180");
    expect(text.entries.map((entry) => entry.text)).toEqual([
      "Standalone Vector180 diagram",
      "Client",
      "Policy service",
    ]);
    expect(
      text.entries.every(
        (entry) =>
          entry.wireFamily === "vector180" &&
          entry.atomId === "system-overview",
      ),
    ).toBe(true);

    const serialized = JSON.stringify({
      outline: outlineAtom(diagram),
      inventory: inventoryAtom(diagram),
      diagram: getAtom(diagram),
      object: getAtomObject(diagram, "system-overview.title"),
      query: queryAtomObjects(diagram, { role: "text" }),
      text,
    });
    expect(serialized).not.toContain("slideId");
    expect(serialized).not.toContain('"slides"');
  });

  it("carries the exact wire family through every legacy atom projection", async () => {
    const atom = await loadAtom({
      kind: "text",
      text: await readLegacyAtom(),
      name: "minimal-diagram.pptv.svg",
    });

    expect(atom.wireFamily).toBe("pptv-legacy");
    expect(outlineAtom(atom).wireFamily).toBe("pptv-legacy");
    expect(inventoryAtom(atom).wireFamily).toBe("pptv-legacy");
    expect(getAtom(atom).wireFamily).toBe("pptv-legacy");
    expect(getAtomObject(atom, "system-overview.title")?.wireFamily).toBe(
      "pptv-legacy",
    );
    expect(
      getAtomObject(atom, "system-overview.title")?.object.wireFamily,
    ).toBe("pptv-legacy");
    expect(
      queryAtomObjects(atom, { role: "text" }).objects.every(
        (object) => object.wireFamily === "pptv-legacy",
      ),
    ).toBe(true);
    expect(extractAtomText(atom).wireFamily).toBe("pptv-legacy");
    expect(
      extractAtomText(atom).entries.every(
        (entry) => entry.wireFamily === "pptv-legacy",
      ),
    ).toBe(true);
  });

  it("suppresses ambiguous diagram object IDs in direct and query projections", async () => {
    const source = (await readMinimalAtom()).replace(
      'id="system-overview.client.label"',
      'id="system-overview.title"',
    );
    const diagram = await loadAtom({ kind: "text", text: source });

    expect(errorCodes(diagram.diagnostics)).toContain("VECTOR180-ID-DUPLICATE");
    expect(getAtomObject(diagram, "system-overview.title")).toBeUndefined();
    expect(
      queryAtomObjects(diagram, { ids: ["system-overview.title"] }).objects,
    ).toEqual([]);
  });
});

describe("standalone diagram preserve patches", () => {
  it("changes one direct-text range and reloads the same artifact kind", async () => {
    const source = await readMinimalAtom();
    const diagram = await loadAtom({ kind: "text", text: source });
    const result = await applyPatch(diagram, {
      schema: "vector180-patch/0.1",
      baseSha256: diagram.source.sha256,
      ops: [
        {
          op: "set-text",
          id: "system-overview.service.label",
          oldText: "Policy service",
          value: "Policy & trust",
        },
      ],
    });

    expect(result.applied).toBe(true);
    expect(result.deck).toBeUndefined();
    expect(result.atom?.sourceKind).toBe("svg");
    expect(result.atom?.id).toBe("system-overview");
    expect(result.sourceText).toBe(
      source.replace(">Policy service</text>", ">Policy &amp; trust</text>"),
    );
    expect(
      getAtomObject(result.atom!, "system-overview.service.label")?.object.text,
    ).toBe("Policy & trust");
  });

  it("preserves a BOM, XML declaration, and every byte outside the text edit", async () => {
    const source = `\uFEFF${await readMinimalAtom()}`;
    const diagram = await loadAtom({ kind: "text", text: source });
    const result = await applyPatch(diagram, {
      schema: "vector180-patch/0.1",
      baseSha256: diagram.source.sha256,
      ops: [
        {
          op: "set-text",
          id: "system-overview.client.label",
          oldText: "Client",
          value: "Caller",
        },
      ],
    });

    expect(result.applied).toBe(true);
    expect(result.sourceText).toBe(
      source.replace(">Client</text>", ">Caller</text>"),
    );
    expect(result.sourceText?.startsWith('\uFEFF<?xml version="1.0"')).toBe(
      true,
    );
    expect(result.atom?.source.bytes.slice(0, 3)).toEqual(
      new Uint8Array([0xef, 0xbb, 0xbf]),
    );
  });

  it("rejects stale and failed text preconditions", async () => {
    const diagram = await loadAtom({
      kind: "text",
      text: await readMinimalAtom(),
    });
    const stale = await validatePatch(diagram, {
      schema: "vector180-patch/0.1",
      baseSha256: "0".repeat(64),
      ops: [
        {
          op: "set-text",
          id: "system-overview.title",
          oldText: "Standalone Vector180 diagram",
          value: "Changed",
        },
      ],
    });
    const precondition = await applyPatch(diagram, {
      schema: "vector180-patch/0.1",
      baseSha256: diagram.source.sha256,
      ops: [
        {
          op: "set-text",
          id: "system-overview.title",
          oldText: "Wrong",
          value: "Changed",
        },
      ],
    });

    expect(errorCodes(stale)).toContain("VECTOR180-PATCH-STALE");
    expect(errorCodes(precondition.diagnostics)).toContain(
      "VECTOR180-PATCH-PRECONDITION",
    );
    expect(precondition.sourceText).toBeUndefined();
    expect(precondition.atom).toBeUndefined();
  });

  it("reconstructs a trusted diagram instead of accepting forged source ranges", async () => {
    const diagram = await loadAtom({
      kind: "text",
      text: await readMinimalAtom(),
    });
    const forgedObjects = new Map(diagram.index.objects);
    const title = forgedObjects.get("system-overview.title")!;
    const client = forgedObjects.get("system-overview.client.label")!;
    forgedObjects.set(title.id, {
      ...title,
      directTextRange: client.directTextRange!,
    });
    const forgedAtom = {
      ...diagram,
      index: { ...diagram.index, objects: forgedObjects },
    };

    const result = await applyPatch(forgedAtom, {
      schema: "vector180-patch/0.1",
      baseSha256: diagram.source.sha256,
      ops: [
        {
          op: "set-text",
          id: "system-overview.title",
          oldText: "Standalone Vector180 diagram",
          value: "Trusted range",
        },
      ],
    });

    expect(result.applied).toBe(true);
    expect(result.sourceText).toContain(">Trusted range</text>");
    expect(result.sourceText).toContain(">Client</text>");
  });

  it("rejects deck-only operations before source ranges and remains atomic", async () => {
    const diagram = await loadAtom({
      kind: "text",
      text: await readMinimalAtom(),
    });
    for (const operation of [
      { op: "set-active-theme", oldTheme: "light", theme: "dark" },
      {
        op: "set-slide-order",
        oldOrder: ["system-overview"],
        order: ["system-overview"],
      },
    ] as const) {
      const result = await applyPatch(diagram, {
        schema: "vector180-patch/0.1",
        baseSha256: diagram.source.sha256,
        ops: [operation],
      });
      expect(errorCodes(result.diagnostics)).toEqual([
        "VECTOR180-PATCH-UNSUPPORTED",
      ]);
      expect(result.sourceText).toBeUndefined();
      expect(result.atom).toBeUndefined();
    }

    const mixed = await applyPatch(diagram, {
      schema: "vector180-patch/0.1",
      baseSha256: diagram.source.sha256,
      ops: [
        {
          op: "set-text",
          id: "system-overview.title",
          oldText: "Standalone Vector180 diagram",
          value: "Would otherwise work",
        },
        { op: "set-active-theme", oldTheme: "light", theme: "dark" },
      ],
    });
    expect(mixed.applied).toBe(false);
    expect(mixed.sourceText).toBeUndefined();
    expect(mixed.affectedIds).toEqual([]);
    expect(mixed.edits).toEqual([]);
    expect(errorCodes(mixed.diagnostics)).toContain(
      "VECTOR180-PATCH-UNSUPPORTED",
    );
  });
});

describe("standalone diagram editor session", () => {
  it("commits text, preserves exact undo/redo, and rejects deck-only intents", async () => {
    const source = `\uFEFF${addVector180AtomDiscoveryComment(
      await readMinimalAtom(),
    )}`;
    const session = await EditorSession.open({
      kind: "text",
      text: source,
      name: "minimal-diagram.vector180.svg",
    });

    expect(session.state.sourceKind).toBe("svg");
    expect(session.state.document.sourceKind).toBe("svg");
    expect(session.state.deck).toBeUndefined();
    expect(session.state.atom?.id).toBe("system-overview");
    expect(session.select("system-overview")).toBe(true);
    expect(session.select("system-overview.service.label")).toBe(true);

    const result = await session.dispatch({
      kind: "set-text",
      id: "system-overview.service.label",
      value: "Authorization",
    });
    expect(result.applied).toBe(true);
    expect(result.atom).toBeDefined();
    expect(session.state.sourceText).toContain(">Authorization</text>");
    expect(session.state.sourceText).toContain(
      VECTOR180_ATOM_DISCOVERY_COMMENT,
    );

    const changedSource = session.state.sourceText;
    const changedHash = session.state.sourceSha256;
    expect(session.undo()).toBe(true);
    expect(session.state.sourceText).toBe(source);
    expect(session.state.sourceSha256).toBe(session.originalSha256);
    expect(session.redo()).toBe(true);
    expect(session.state.sourceText).toBe(changedSource);
    expect(session.state.sourceSha256).toBe(changedHash);

    const historySource = session.state.sourceText;
    const unsupported = await session.dispatch({
      kind: "set-active-theme",
      theme: "dark",
    });
    expect(errorCodes(unsupported.diagnostics)).toEqual([
      "VECTOR180-EDITOR-DOCUMENT-KIND",
    ]);
    expect(session.state.sourceText).toBe(historySource);
  });

  it("fails closed on an integrity mismatch without changing history", async () => {
    const session = await EditorSession.open(
      { kind: "text", text: await readMinimalAtom() },
      { expectedSha256: "0".repeat(64) },
    );
    const result = await session.dispatch({
      kind: "set-text",
      id: "system-overview.title",
      value: "Blocked",
    });

    expect(session.state.editable).toBe(false);
    expect(errorCodes(result.diagnostics)).toEqual([
      "VECTOR180-EDITOR-READ-ONLY",
    ]);
    expect(session.state.canUndo).toBe(false);
  });
});
