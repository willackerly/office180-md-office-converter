...
import type { Vector180ConcreteNativeStyle } from "../core/types.js";
import { applyPatch, validatePatch } from "../ops/patch.js";
import { errorCodes } from "./test-helpers.js";

const RECT_STYLE: Vector180ConcreteNativeStyle = {
  fill: "#112233",
  stroke: "#445566",
  strokeWidth: 2,
  opacity: 0.8,
...
function diagramSource(): string {
  ...
  <rect id="rect-a" data-vector180-role="shape" data-vector180-export="native"
    x="20" y="30" width="100" height="60"
    fill = '#112233' stroke="#445566" stroke-width="2" opacity=".8"/>
  <ellipse id="ellipse-b" data-vector180-role="shape" data-vector180-export="native"
  ...
  <line id="connector" data-vector180-role="connector" data-vector180-export="native"
    ...
    data-vector180-from="rect-a" data-vector180-to="ellipse-b"
    fill="none" stroke="#334455" stroke-width="3"/>
  <g id="group" data-vector180-role="group" data-vector180-export="native"
    transform="translate(10, 20)">
    <rect id="group-box" data-vector180-role="shape" data-vector180-export="native"
      x="400" y="250" width="180" height="100" fill="#eeeeee"/>
    <text id="group-label" data-vector180-role="text" data-vector180-export="native"
      data-vector180-frame="420 270 140 50" data-vector180-line-step="24"
...
describe("Vector180 unified typed patch 0.1", () => {
  it("accepts text and typed geometry operations in one canonical envelope", async () => {
    ...
    expect(result.sourceText).toContain(">Unified vocabulary</text>");
    expect(result.sourceText).toContain(
      `x="25" y="30" width="100" height="60"`,
    );
  });
  ...
  it("patches rect and true ellipse geometry through existing attributes", async () => {
    ...
    expect(result.sourceText).toContain('data-vector180-from="rect-a"');
  ...
  it("patches connector endpoints without touching stable references", async () => {
    ...
    expect(result.sourceText).toContain('data-vector180-to="ellipse-b"');
  });
  ...
  it("moves a direct text frame and its explicit hard-line anchor together", async () => {
    ...
    expect(result.applied).toBe(true);
    expect(result.sourceText).toContain(
      'data-vector180-frame="430 280 150 55"',
    );
    ...
    const mixed = await loadAtom({ kind: "text", text: mixedSource });
    const rejected = await applyPatch(
      mixed,
  ...
  it("reorders exact child element slots within a root or group", async () => {
    ...
    expect(rootResult.atom?.children.map((node) => node.id)).toEqual([
      "ellipse-b",
      "rect-a",
      "connector",
      "group",
  ...
  it("deletes safe native objects and accounts for same-transaction connector deletion", async () => {
    ...
    expect(errorCodes(blocked.diagnostics)).toContain(
    ...
    expect(result.applied).toBe(true);
    expect(result.atom?.index.objects.has("connector")).toBe(false);
    expect(result.atom?.index.objects.has("ellipse-b")).toBe(false);
    ...
    expect(errorCodes(root.diagnostics)).toContain(
  ...
  it("patches concrete style only through direct presentation attributes", async () => {
    ...
    const inherited = await loadAtom({
      kind: "text",
      text: inheritedSource,
    });
    const inheritedModel = resolveVector180Atom(inherited).model;
  ...
  it("rejects stale old values, invalid numbers, and overlapping typed edits atomically", async () => {
    ...
    expect(errorCodes(invalid)).toContain("VECTOR180-PATCH-SCHEMA");
    ...
    expect(errorCodes(overlap.diagnostics)).toContain(
  ...
  it("rejects circle representation and C6-invalid candidate text placement", async () => {
    ...
    const circle = await loadAtom({ kind: "text", text: circleSource });
    expect(resolveVector180Atom(circle).model).toBeDefined();
