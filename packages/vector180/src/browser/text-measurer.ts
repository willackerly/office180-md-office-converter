...
export type Vector180BrowserTextMeasureRequest =
  Vector180DeckTextMeasureRequest | Vector180AtomTextMeasureRequest;

export interface Vector180BrowserGlyphCoverage {
...
export interface Vector180BrowserFontSource {
   ...
   */
  readonly coverage?: Vector180BrowserGlyphCoverage;
}

export interface Vector180BrowserEnvironmentEvidence {
...
export interface Vector180BrowserLoadedFontEvidence {
  readonly family: string;
  readonly weight: 400 | 700;
  readonly style: "normal" | "italic";
...
export interface Vector180PreparedBrowserTextMeasurer {
  readonly environment: Vector180BrowserEnvironmentEvidence;
  readonly fonts: readonly Vector180BrowserLoadedFontEvidence[];
  readonly measure: (
    request: Vector180BrowserTextMeasureRequest,
  ) => Vector180TextMeasurement;
  readonly dispose: () => void;
}

export interface PrepareVector180BrowserTextMeasurerOptions {
...
interface LoadedFace {
  ...
  readonly evidence: Vector180BrowserLoadedFontEvidence;
  readonly coverage?: {
    readonly method: string;
 ...
 */
export async function prepareVector180BrowserTextMeasurer(
  ...
  try {
    for (const [index, source] of normalizedSources.entries()) {
      ...
      const evidence: Vector180BrowserLoadedFontEvidence = Object.freeze({
...
export function browserEnvironmentFromUserAgent(
  ...
  platform = "unknown",
  devicePixelRatio = 1,
): Vector180BrowserEnvironmentEvidence {
  if (
...
export function browserFontAlias(
  ...
  return `Vector180_${sha256.slice(0, 24)}_${weight}_${style}_${index}`;
}

function validateSources(
  sources: readonly Vector180BrowserFontSource[],
): readonly Vector180BrowserFontSource[] {
...
function normalizeCoverage(coverage: Vector180BrowserGlyphCoverage): {
  readonly method: string;
  readonly checked: ReadonlySet<number>;
  readonly missing: ReadonlySet<number>;
} {
...
function validateRequest(request: Vector180BrowserTextMeasureRequest): void {
  if (
  ...
  const hasAtom =
    ...
    request.atomId.length > 0;
  if (hasSlide === hasAtom) {
    throw new Error(
      "Browser text measurement requires exactly one slideId or atomId.",
    );
  }
}
