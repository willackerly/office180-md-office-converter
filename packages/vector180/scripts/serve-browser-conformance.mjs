/**
 * Minimal fixed-route HTTP harness for real-browser Vector180 conformance tests.
 *
...
const port = Number.parseInt(
  process.env["Vector180_CONFORMANCE_PORT"] ?? "4178",
  10,
);
if (!Number.isInteger(port) || port <= 0 || port > 65535) {
  throw new Error("Vector180_CONFORMANCE_PORT must be a valid TCP port.");
}
...
<head>
  ...
  <title>Vector180 browser conformance</title>
</head>
<body>
  <p id="status">Vector180 browser kernel harness</p>
  <script src="/assets/vector180-browser-kernel-0.1.iife.js"></script>
</body>
</html>
`;
...
server.listen(port, host, () => {
  process.stdout.write(
    `Vector180 browser conformance server listening on http://${host}:${port}\n`,
  );
});
