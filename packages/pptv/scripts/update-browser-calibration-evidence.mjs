...
const EVIDENCE_SCHEMA = "pptv-browser-text-calibration-evidence/0.1";
const CAPTURE_SCHEMA = "pptv-browser-text-calibration-capture/0.1";
...
async function loadContext() {
  const [
    ...
    readJsonUrl(
      new URL("../assets/pptv-browser-kernel-0.1.meta.json", import.meta.url),
  ...
  const pptv = asRecord(pptvPackage, "PPTV package.json");
  const rootDevDependencies = asRecord(
    root.devDependencies,
    "root devDependencies",
  );
  const pptvDependencies = asRecord(pptv.dependencies, "PPTV dependencies");
  ...
  const fontkit = exactPackageVersion(pptvDependencies.fontkit, "fontkit");
...
function validateFontManifest(value, asset) {
  ...
  return {
    ...
    weight,
    style,
    alias: `Pptv_${sha256.slice(0, 24)}_${weight}_${style}_0`,
    sha256,
...
function validateCalibration(value) {
  ...
  assertEqual(
    calibration.schema,
    "pptv-browser-text-calibration/0.1",
    "calibration schema",
  );
  assertEqual(
