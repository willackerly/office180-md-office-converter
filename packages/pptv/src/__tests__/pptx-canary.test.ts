...
async function canaryModel(): Promise<PptvResolvedDeck> {
  const deck = await loadDeck({
    kind: "text",
    text: await readMinimalDeck(),
    name: "canary.pptv.html",
  });
  ...
  return model as unknown as PptvResolvedDeck;
}
...
describe("C7 deterministic PPTX compiler canary", () => {
  it("builds a sorted STORE-only two-slide package with a complete strict graph", async () => {
    ...
    const first = await compilePptxCanary(model);
    const second = await compilePptxCanary(model);
    expect(first.bytes).toEqual(second.bytes);
    expect(first.partNames).toEqual([...first.partNames].sort());
    expect(first.partNames).toEqual([
      "[Content_Types].xml",
      "_rels/.rels",
      "docProps/app.xml",
      "docProps/core.xml",
    ...
    expect(headers.map((header) => header.name)).toEqual(first.partNames);
    ...
    expect(custom).toContain("office180-pptv-pptx-canary/0.1");
  });
  ...
  it("writes a canonical raw central directory without ZIP extensions", async () => {
    ...
    expect(central.diskEntries).toBe(artifact.partNames.length);
    expect(central.totalEntries).toBe(artifact.partNames.length);
    ...
    expect(local.map((entry) => entry.name)).toEqual(artifact.partNames);
  ...
  it("retains manifest order, painter order, stable names, native groups, and exact text intent", async () => {
    ...
    expect(cover).toContain("<a:t>Minimal PPTV deck</a:t>");
  ...
  it("is byte-identical in separate Node processes and timezones", async () => {
    ...
    const script = `
      ...
      import { loadDeck } from "./packages/pptv/src/core/deck.ts";
      import { resolvePptvDeck } from "./packages/pptv/src/core/resolved.ts";
      import { compilePptxCanary } from "./packages/pptv/src/node/pptx-canary.ts";
      const source = await readFile("examples/minimal-deck.pptv.html", "utf8");
  ...
  const rejectedRanges: readonly RejectedRangeCase[] = [
    {
      ...
      code: "PPTV-PPTX-UNSUPPORTED-GEOMETRY",
    },
    {
  ...
  it.each(rejectedRanges)("rejects $label", async ({ mutate, code }) => {
    ...
    await expectAsyncCanaryCode(
      compilePptxCanary(model as unknown as PptvResolvedDeck),
      code,
    );
  });
  ...
  it("fails closed on unresolved, asset, rounded, multiline, opacity, and fractional mappings", async () => {
    ...
    await expectAsyncCanaryCode(
      compilePptxCanary(asset as unknown as PptvResolvedDeck),
    ...
    await expectAsyncCanaryCode(
      compilePptxCanary(rounded as unknown as PptvResolvedDeck),
    ...
    await expectAsyncCanaryCode(
      compilePptxCanary(multiline as unknown as PptvResolvedDeck),
      "PPTV-PPTX-UNSUPPORTED-OBJECT",
    );
    ...
    await expectAsyncCanaryCode(
      compilePptxCanary(translucent as unknown as PptvResolvedDeck),
      "PPTV-PPTX-UNSUPPORTED-GEOMETRY",
    );
    ...
    await expectAsyncCanaryCode(
      compilePptxCanary(fractional as unknown as PptvResolvedDeck),
    ...
    await expectAsyncCanaryCode(
      compilePptxCanary(fractionalFont as unknown as PptvResolvedDeck),
  ...
  it("rejects forged geometry and malformed OPC graphs", async () => {
    ...
    await expectAsyncCanaryCode(
      compilePptxCanary(forged as unknown as PptvResolvedDeck),
    ...
    await expectAsyncCanaryCode(
      compilePptxCanary(colliding as unknown as PptvResolvedDeck),
      "PPTV-PPTX-ID-COLLISION",
    );
