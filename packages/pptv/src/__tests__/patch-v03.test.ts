...
function diagramSource(): string {
  ...
  <g id="group" data-pptv-role="group" data-pptv-export="native">
    <rect id="group-a" data-pptv-role="shape" data-pptv-export="native"
      x="400" y="220" width="100" height="60" fill="#eeeeee"/>
    <rect id="group-b" data-pptv-role="shape" data-pptv-export="native"
      x="620" y="220" width="100" height="60" fill="#eeeeee"/>
    <line id="group-connector" data-pptv-role="connector"
      data-pptv-export="native" x1="500" y1="250" x2="620" y2="250"
      data-pptv-from="group-a" data-pptv-to="group-b"
      fill="none" stroke="#334455" stroke-width="3" opacity="1"
      font-weight="400" font-style="normal" text-anchor="start"/>
  </g>
</svg>`;
...
async function loadFixture(source = diagramSource()): Promise<PptvDiagram> {
...
describe("PPTV connector clone patch 0.3", () => {
  it.each([
    {
    ...
    expect(result.diagram?.children.map((child) => child.id)).toEqual(order);
    const clone = resolvePptvDiagram(result.diagram!).model?.objects.find(
    ...
    expect(result.sourceText).toContain(
      "<line id='connector' data-pptv-role='connector'",
    );
    expect(result.sourceText).toContain(
      "<line id='connector-copy' data-pptv-role='connector'",
    );
    expect(result.sourceText).toContain(
      "data-pptv-from='node-b' data-pptv-to='group'",
    );
  ...
  it("preserves 0.1/0.2 and rejects clone-connector outside 0.3", async () => {
    const diagram = await loadFixture();
    const legacy = await applyPatch(
      diagram,
      patch(
        diagram,
        [
    ...
    const wrongEnvelope = await applyPatch(
      diagram,
      patch(
        diagram,
        [
  ...
  it("requires exactly one clone operation in every 0.3 envelope", async () => {
    const diagram = await loadFixture();
    ...
    expect(errorCodes(noClone)).toContain("PPTV-PATCH-SCHEMA");
    expect(errorCodes(twoClones)).toContain("PPTV-PATCH-SCHEMA");
  });
  ...
  it("rejects stale state, reused IDs, missing references, and non-insertion order", async () => {
    ...
    expect(errorCodes(reused.diagnostics)).toContain("PPTV-PATCH-PRECONDITION");
  ...
  it("rejects non-whitespace insertion slots and non-direct style materialization", async () => {
    ...
    const missingLiteral = await loadFixture(
      diagramSource().replace(" data-pptv-to='node-b'", ""),
    );
