...
export type PptxCanaryErrorCode =
  | "PPTV-PPTX-UNRESOLVED"
  | "PPTV-PPTX-INVALID-MODEL"
  | "PPTV-PPTX-UNSUPPORTED-OBJECT"
  | "PPTV-PPTX-UNSUPPORTED-GEOMETRY"
  | "PPTV-PPTX-NON-INTEGRAL-EMU"
  | "PPTV-PPTX-NON-INTEGRAL-FONT"
  | "PPTV-PPTX-ID-COLLISION"
  | "PPTV-PPTX-OPC-GRAPH"
  | "PPTV-PPTX-ZIP-LIMIT";
...
export interface PptxCanaryArtifact {
  readonly bytes: Uint8Array;
...
export async function compilePptxCanary(
  ...
  if (packageParts.length > 65_535) {
    fail("PPTV-PPTX-ZIP-LIMIT", "The canary package exceeds ZIP32 entries.");
  }
  ...
  return Object.freeze({
    bytes,
    partNames: Object.freeze(packageParts.map((part) => part.name)),
...
function validateResolvedDeck(model: PptvResolvedDeck): void {
  if (
    model.schema !== "pptv-resolved/0.1" ||
  ...
  if (model.slides.length === 0) {
    fail("PPTV-PPTX-INVALID-MODEL", "A PPTX requires at least one slide.");
  }
...
function validateObjectGeometry(object: PptvResolvedObject): void {
  switch (object.kind) {
    case "rect":
...
function validateGroupBounds(object: PptvResolvedGroup): void {
...
function validateStyle(style: PptvResolvedStyle, objectId: string): void {
  if (
...
function renderAppProperties(model: PptvResolvedDeck): string {
  ...
  <Application>office180 PPTV</Application>
...
function renderTheme(): string {
  return xmlDocument(`<a:theme xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" name="PPTV Canary">
  <a:themeElements>
    <a:clrScheme name="PPTV Canary">
    ...
    <a:fontScheme name="PPTV Canary">
      <a:majorFont><a:latin typeface="Arial"/><a:ea typeface=""/><a:cs typeface=""/></a:majorFont>
      <a:minorFont><a:latin typeface="Arial"/><a:ea typeface=""/><a:cs typeface=""/></a:minorFont>
    </a:fontScheme>
    <a:fmtScheme name="PPTV Canary">
      <a:fillStyleLst>
...
function renderSlide(slide: PptvResolvedSlide): string {
...
function renderEllipse(
  object: Extract<PptvResolvedObject, { readonly kind: "ellipse" }>,
  context: RenderContext,
  indent: string,
): string {
...
function renderFill(style: PptvResolvedStyle, indent: string): string {
  return renderPaint(style.fill, style.opacity, "fill", indent);
}

function renderLineStyle(style: PptvResolvedStyle, indent: string): string {
...
function renderTextOutline(style: PptvResolvedStyle, indent: string): string {
...
function scaledInteger(
  ...
  if (!Number.isFinite(value)) {
    fail("PPTV-PPTX-INVALID-MODEL", `${label} must be finite.`);
  }
...
function validateBounds(bounds: PptvBounds, label: string): void {
...
function sameBounds(left: PptvBounds, right: PptvBounds): boolean {
  return (
...
function invalidModel(message: string): never {
  return fail("PPTV-PPTX-INVALID-MODEL", message);
}

function graphFail(message: string): never {
  return fail("PPTV-PPTX-OPC-GRAPH", message);
}
