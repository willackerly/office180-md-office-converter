...
import type {
  PptvBrowserFontSource,
  PptvBrowserTextMeasureRequest,
  PptvPreparedBrowserTextMeasurer,
} from "../src/browser/text-measurer.js";

interface BrowserKernel {
  inspectPptvConformance(input: {
    kind: "text";
    text: string;
    name: string;
  ...
  resolvePptvDiagram(diagram: PptvDiagram): PptvResolvedDiagramResult;
  preflightDiagramTextFit(
    diagram: NonNullable<PptvResolvedDiagramResult["model"]>,
...
const INVALID_PROFILE_URL = new URL(
  "../test-fixtures/c6/invalid-profile.pptv.svg",
  import.meta.url,
);
...
const fixtures = [
  ...
  {
    name: "kitchen-sink.pptv.svg",
    url: KITCHEN_SINK_URL,
  },
  {
    name: "invalid-profile.pptv.svg",
...
for (const fixture of fixtures) {
  test(`C4/C6 normalized parity: ${fixture.name}`, async ({ page }) => {
    ...
    const nodeResult = await inspectPptvConformance({
      kind: "text",
      text: source,
      name: fixture.name,
    });
    const browserResult = await page.evaluate(
      async ({ name, text }) => {
        const kernel = (
          globalThis as unknown as { PptvBrowserKernel: BrowserKernel }
        ).PptvBrowserKernel;
        return kernel.inspectPptvConformance({ kind: "text", text, name });
      },
...
test("C8 loads exact bytes, labels the environment, and fails closed on missing glyphs", async ({
  ...
  const result = await page.evaluate(
    async (fixture) => {
      ...
      const prepared = await kernel.preparePptvBrowserTextMeasurer([
        {
      ...
      const browserRows = fixture.nodeRows.map((sample, lineIndex) => {
        ...
        } satisfies PptvBrowserTextMeasureRequest);
      ...
      const diagram = await kernel.loadDiagram({
        kind: "text",
        text: fixture.diagramSource,
        name: "kitchen-sink.pptv.svg",
      });
      const resolvedDiagram = kernel.resolvePptvDiagram(diagram);
      if (resolvedDiagram.model === undefined) {
        throw new Error(
          `diagram resolution failed: ${JSON.stringify(resolvedDiagram.diagnostics)}`,
        );
      }
      const diagramFit = kernel.preflightDiagramTextFit(
        resolvedDiagram.model,
        prepared.measure,
      );
      const result = {
