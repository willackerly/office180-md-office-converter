/*! @office180/pptv writable editor 0.1 | generated by esbuild 0.28.1 */
"use strict";
(() => {
...
`;
  ...
  async function scanPptvSource(input, options = {}) {
    ...
    if (kind === "html") {
      return await scanHtml(document2, mapper, diagnostics, options);
    }
    if (kind === "svg") {
      ...
      return scanSvg(document2, mapper, diagnostics, options);
    }
    return {
      kind,
      encoding: "utf-8",
  ...
  function diagnosticScope(document2, scopeId) {
    return document2.sourceKind === "html" ? { slideId: scopeId } : { diagramId: scopeId };
  }
    ...
    if (intent.kind === "set-active-theme") {
      return {
        operation: {
          op: "set-active-theme",
          theme: intent.theme,
          ...document2.sourceKind === "html" && document2.activeTheme !== void 0 ? { oldTheme: document2.activeTheme } : {}
        },
        diagnostics: []
      };
    }
    if (intent.kind === "set-slide-order") {
      return {
        operation: {
          op: "set-slide-order",
          order: [...intent.order],
          ...document2.sourceKind === "html" ? { oldOrder: [...document2.slideOrder] } : {}
        },
        diagnostics: []
      };
    }
  ...
  var nodes = {
    payload: required("#pptv-editor-payload"),
  ...
  async function bootstrap() {
    ...
    if ((session.state.sourceKind === "html" ? "deck" : "diagram") !== payload.documentKind) {
  ...
  function sameLineEvidence(left, right) {
    return left.objectId === right.objectId && left.lineIndex === right.lineIndex && left.text === right.text && left.anchor === right.anchor && left.anchorX === right.anchorX && left.frameX === right.frameX && left.frameWidth === right.frameWidth && left.availableWidth === right.availableWidth && left.font.family === right.font.family && left.font.size === right.font.size && left.font.weight === right.font.weight && left.font.style === right.font.style && ("slideId" in left ? "slideId" in right && left.slideId === right.slideId : "diagramId" in right && left.diagramId === right.diagramId);
  }
  ...
  function renderViewport() {
    ...
    svg.setAttribute(
      "aria-label",
      view.kind === "deck" ? `Resolved slide ${selectedSlideId ?? ""}` : `Resolved diagram ${view.outline.diagramId}`
    );
    ...
    try {
      if (fileHandle === void 0) {
        fileHandle = await picker({
          ...
          types: [
            {
              description: payload.documentKind === "deck" ? "PPTV HTML deck" : "PPTV SVG diagram",
              accept: {
  ...
  function findSelectedNode(state) {
    ...
    const roots = state.sourceKind === "html" ? [...state.deck.slides.values()].flatMap(({ children }) => children) : state.diagram.children;
