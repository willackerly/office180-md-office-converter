# Vector180 HTML Container

**Status:** `@office180/vector180@0.1.0-alpha.5` implements the source/container 0.1
deck and diagram surfaces through C4-C8, C5 2.0 typed patches including one
exact reviewed connector clone, bounded C9 atom composition/PPTX compilation,
and C10 2.0 mapped-PPTX reconciliation. Repository C11 1.2 tooling adds
...
Vector180 HTML is the declarative, single-file aggregation envelope for a deck of
Vector180 SVG slides. It keeps canonical slide content web-native and
...
the default authoring source is a fully hydrated independent `*.vector180.svg`, not
...
```text
mydeck.vector180.html
```

A future multi-file assembly manifest reserves the convention:

```text
mydeck.vector180-manifest.json
```

The manifest is currently inventory-only orchestration metadata, not a
semantic load/edit surface or a JSON encoding of Vector180 itself.

## Implemented source/profile 0.1 boundary

In package `0.1.0-alpha.5`, `@office180/vector180` implements C4-C6 for both
self-contained HTML decks and standalone SVG diagrams. Both paths retain exact
...
source code. C5 2.0's one `vector180-patch/0.1` envelope covers direct text,
deck-only active-theme/slide-order transactions, typed native-object
operations, and the narrow `clone-connector` exception.
...
The `extractVector180Atom()` API, `vector180 extract` CLI, and editor download action
hydrate one valid deck slide into an independent `.vector180.svg`: supported
...
placement to a mapped native PPTX. C10 2.0 reconciles only an edited descendant
...
```text
diagram.vector180.svg              # default independent diagram atom
mydeck.vector180.html              # one portable browser-viewable deck
mydeck.vector180-manifest.json     # reserved inventory-only external assembly
```
...
Strict `.vector180.html` sources use this physical order:

1. minimal HTML metadata;
2. one Vector180 manifest data block;
...
<svg id="system-overview"
  ...
  <text id="system-overview.api.title"
        ...
        data-vector180-export="native"
        data-vector180-frame="110 130 300 60"
...
<template data-vector180-slide="cover">
  <svg id="cover" viewBox="0 0 1600 900" data-vector180-layout="title">
    <text
      id="cover.title"
      class="slide-title"
      data-vector180-role="text"
...
<template data-vector180-slide="system-overview">
  <svg id="system-overview" viewBox="0 0 1600 900"
       data-vector180-layout="architecture">
    <!-- semantic slide objects -->
  </svg>
</template>

<template data-vector180-slide="trust-boundaries">
...
<script type="text/css" data-vector180-theme="dapple.light">
:root {
  --vector180-background: #ffffff;
  --vector180-text-primary: #17211e;
  --vector180-surface-raised: #f7f8f8;
  --vector180-accent-1: #6f5cff;
  --vector180-font-major: "Inter Display";
}
...
referenced source. Vector180 should avoid two sources of truth.
...
```bash
pnpm vector180 extract deck.vector180.html \
  --slide architecture \
...
```html
<template data-vector180-library="dapple.components">
...
<script type="text/css" data-vector180-theme="dapple.dark">
:root {
  --vector180-background: #101714;
  --vector180-text-primary: #f5f7f6;
  --vector180-accent-1: #9a8cff;
}
</script>
```
...
does not yet emit native PowerPoint theme bindings; the C7 and bounded C9
writers use concrete resolved colors/fonts.
...
possible. A value that originates from a Vector180 theme token should become a
...
future compiler behavior; the current C7/C9 native subsets write concrete
resolved colors and fonts.
...
The runtime is **not authoritative**. Vector180 compilers and validators must never
...
`vector180-browser/0.1` runtime only when its inline content matches the installed
...
opening an untrusted `.vector180.html` executes its script before library validation.
...
## Logical normalization (implemented C6 2.0 subset; acceptance pending)

The normalized Vector180 object model is a resolver/editor/compiler representation,
...
For source/profile 0.1, the alpha.5 implementation exposes outline, text,
semantic, editing, and C6 resolved projections for either a self-contained
deck or standalone diagram.
...
agent. Vector180 tooling should expose progressively richer projections.

### Outline view

```bash
pnpm vector180 outline dapple-overview.vector180.html
pnpm vector180 outline system-overview.vector180.svg
```
...
```bash
pnpm vector180 show dapple-overview.vector180.html \
  trust-boundaries --view semantic

pnpm vector180 show system-overview.vector180.svg \
...
Agents should normally edit through the stable-ID/source-bound C5 2.0
...
The implementation's one `vector180-patch/0.1` envelope also supports:
...
creating synthetic deck state. Every typed operation carries its complete
old C6 value, requires one unambiguous existing source representation, and
reloads and resolves the complete same-kind candidate before success. The
typed surface covers rect/ellipse geometry, line endpoints, an existing
...
The envelope permits `clone-connector` at most once. It exact-clones one
...
A Vector180 source may declare a known agent profile:
...
with the Vector180 toolchain. The current standalone SVG root allowlist does not
...
Implemented by `@office180/vector180@0.1.0-alpha.5` over source/profile 0.1:
...
`outline`, `validate`, `resolve`, `text`, `show`, `list`, `patch`, `text-fit`,
and `editor-pack` accept either `.vector180.html` or `.vector180.svg`.
Deck-only options fail explicitly for an atom. `metadata`, `metadata-compare`,
and `diff` expose bounded atom evidence; `migrate` is the only general legacy
PPTV-to-Vector180 write and accepts one complete legacy SVG atom. `new atom`
and `new deck` create validation-locked canonical starters without overwriting
an existing destination. `patch` requires exactly one of `--check` or an
explicit `--output`; `extract`, `editor-pack`, and `pptx-canary` also require
explicit atomic destinations. `text-fit` uses the verified bundled ABeeZee map
when `--font-map` is omitted; a custom exact map remains explicit.
`extract` accepts an HTML deck plus one slide ID and emits a standalone SVG.
`pptx-canary` accepts HTML decks only and never wraps an SVG into a synthetic
slide, even when its viewBox is 16:9.

`compose` and `compile` are the bounded C9 standalone-atom path. Both require
an explicit `X,Y,W,H` placement on a `1600 × 900` slide and accept either
identity or an explicitly selected exact-aspect uniform scale plus translation.
`compose` writes one deterministic self-contained deck; `compile` atomically
writes the native PPTX and its complete hash-bound sidecar map. They do not
accept deck input or infer physical size.
...
wholly patchable supported result. Ordinary edits use `vector180-patch/0.1`.
Duplicates refuse by default. A `vector180-reconcile-resolution/0.1` document may
produce one `vector180-patch/0.1` connector clone only when exactly one of exactly
...
options. Applying an accepted proposal is a separate `vector180 patch` action.
...
vector180 bundle
vector180 explode
vector180 agent-guide
```

General deck/multi-atom composition, deck-mode C9 compilation, and
baseline-free PowerPoint import remain capability work, not alternate meanings
of the implemented `compose`, `compile`, or `reconcile` commands.
...
`*.editable.html` convenience artifact for either source kind:

```bash
pnpm vector180 editor-pack diagram.vector180.svg \
  --output diagram.editable.html \
  --font-map default
```
...
Those are the browser UI's only write controls. The portable C5 kernel can
apply typed geometry, connector, explicit group translation, direct
one-line text-frame, child-order, safe-deletion, and direct native-style
transactions programmatically, and C10 may propose them, but the editor does
not expose corresponding gestures or property controls.

Text remains strictly no-reflow. Neither C5 patches, the editor, nor C6-C10
wrap, autofit, shrink, or re-line authored text. The selected exact map is the
bundled verified default when the option is omitted, or a caller-supplied map
when explicit. The pack embeds only those selected exact font bytes, their
identity, and matching Node C8 evidence; the browser measures the same bytes
and labels its engine/version. The editor displays the worse current
Node/browser status. If an edit invalidates the embedded Node evidence, that
line becomes `unverified` until exact evidence is recomputed, even when the
browser can measure it. This is conservative overflow evidence, not
browser/native PowerPoint parity or automatic repair.

## Implemented conformance boundary

A `.vector180.html` deck is conforming when:
...
A `.vector180.svg` diagram is conforming when:

- its single SVG root declares a stable `id`, `data-vector180-version="0.1"`, the SVG
...
## Relationship to the Vector180 SVG profile

`VECTOR180-PROFILE.md` defines the constrained SVG object model intended for
...
- reverse conversion must produce a reviewable semantic patch; C10 implements
...
C5 2.0, and C6. C7 remains a strict fresh-PPTX canary for HTML decks, C8 adds
