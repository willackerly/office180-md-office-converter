...
/**
 * Generate a privacy-safe, content-bound Vector180/PPTX round-trip evidence bundle.
 *
...
const requireFromVector180 = createRequire(
  new URL("../packages/vector180/package.json", import.meta.url),
);
const JSZip = requireFromVector180("jszip");
...
const SOURCE_METADATA =
  '{"styleFamily":{"id":"office180.vector180.roundtrip","version":"1.0"}}';
...
<svg id="roundtrip"
  ...
  <title>Supported Vector180 round trip</title>
  <desc>Synthetic public fixture for text, geometry, and style reconciliation.</desc>
  <metadata data-vector180-metadata="vector180-atom-metadata/0.1">${SOURCE_METADATA}</metadata>
  ...
  <text id="${TITLE_ID}"
        ...
        data-vector180-export="native"
        data-vector180-frame="55 42 690 70"
        data-vector180-line-step="46"
        x="400" y="93"
  ...
  <text id="roundtrip.subtitle"
        data-vector180-role="text"
        data-vector180-export="native"
        data-vector180-frame="110 116 580 42"
        data-vector180-line-step="28"
        x="400" y="146"
  ...
  <text id="roundtrip.card.label"
        data-vector180-role="text"
        data-vector180-export="native"
        data-vector180-frame="100 225 250 48"
        data-vector180-line-step="30"
        x="225" y="260"
  ...
  <text id="roundtrip.card.detail"
        data-vector180-role="text"
        data-vector180-export="native"
        data-vector180-frame="92 302 266 46"
        data-vector180-line-step="26"
        x="225" y="334"
  ...
  <line id="roundtrip.connector"
        ...
        data-vector180-export="native"
        data-vector180-from="${CARD_ID}"
        data-vector180-to="roundtrip.result"
  ...
  <text id="roundtrip.result.label"
        data-vector180-role="text"
        data-vector180-export="native"
        data-vector180-frame="540 270 160 58"
        data-vector180-line-step="30"
        x="620" y="309"
  ...
  <text id="roundtrip.footer.label"
        data-vector180-role="text"
        data-vector180-export="native"
        data-vector180-frame="120 508 560 38"
        data-vector180-line-step="25"
        x="400" y="536"
        fill="#334155" stroke="none" stroke-width="0"
...
function runVector180(args, stage, expectedStatuses = [0]) {
  return run(
    "pnpm",
    ["--silent", "vector180", ...args],
    stage,
    expectedStatuses,
  );
}

function runVector180Json(args, stage, expectedStatuses = [0]) {
  return parseCommandJson(
    runVector180([...args, "--format", "json"], stage, expectedStatuses),
    `vector180 ${args[0]}`,
  );
}
...
async function assertMap(mapPath, sourcePath, composedPath, pptxPath) {
  ...
  const [sourceSha, composedSha, pptxSha, composedSource] = await Promise.all([
    sha256File(sourcePath),
    sha256File(composedPath),
    sha256File(pptxPath),
    readFile(composedPath, "utf8"),
  ]);
  assert(map.schema === "vector180-pptx-map/0.1", "unexpected C9 map schema");
  assert(
    map.source?.family === "vector180" &&
      map.source.kind === "atom" &&
      map.source.id === "roundtrip" &&
      map.source.profile === "0.1",
    "C9 map source identity mismatch",
  );
  assert(map.source?.sha256 === sourceSha, "C9 map atom hash mismatch");
  assert(
    map.source?.metadataSha256 === sha256Bytes(SOURCE_METADATA),
    "C9 map metadata hash mismatch",
  );
  assert(
    !composedSource.includes("data-vector180-metadata"),
    "C9 composition copied atom-only metadata into generated deck source",
  );
  assert(
...
async function runTextFit(sourcePath, fontMapPath, outputPath, stage) {
  const result = runVector180Json(
    ["text-fit", sourcePath, "--font-map", fontMapPath],
    stage,
  );
  assert(
    result.schema === "vector180-text-fit-atom/0.1",
...
function readmeText({
  ...
  return `# Vector180/PPTX round-trip evidence
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (value === "--help" || value === "-h") {
      process.stdout.write(
        "Usage: node scripts/generate-vector180-roundtrip-evidence.mjs [--destination PATH]\n",
      );
      process.exit(0);
    }
...
async function generate(destination) {
  ...
  const prefix = join(dirname(destination), ".vector180-evidence.");
  const stage = await mkdtemp(prefix);
  let published = false;
  try {
    assert(
      dirname(stage) === dirname(destination) &&
        basename(stage).startsWith(".vector180-evidence."),
      "unsafe evidence staging directory",
    );
    const sourcePath = join(stage, "supported-original.vector180.svg");
    const fontMapPath = join(stage, "supported-font-map.json");
    const composedPath = join(stage, "supported-composed.vector180.html");
    const baselinePptx = join(stage, "supported-baseline.pptx");
    const baselineMapPath = join(
      stage,
      "supported-baseline.vector180.map.json",
    );
    const editedPptx = join(stage, "supported-edited.pptx");
    const reconcileReportPath = join(stage, "supported-edited.reconcile.json");
    const patchPath = join(stage, "supported-recovered.vector180.patch.json");
    const recoveredPath = join(stage, "supported-recovered.vector180.svg");
    const regeneratedPptx = join(stage, "supported-regenerated.pptx");
    const regeneratedMapPath = join(
      stage,
      "supported-regenerated.vector180.map.json",
    );
    const regeneratedComposedPath = join(
      stage,
      ".supported-regenerated.composed.vector180.html",
    );
    ...
    assert(
      fontRelative ===
        "../../../../packages/vector180/test-fixtures/fonts/ABeeZee-Regular.ttf",
    ...
    const originalValidation = runVector180Json(
      ["validate", sourcePath],
      stage,
    );
    assert(
    ...
    const composeSummary = runVector180Json(
      [
        "compose",
        sourcePath,
        "--placement",
    ...
    const baselineCompile = runVector180Json(
      [
        "compile",
        sourcePath,
        "--placement",
    ...
    assert(
      patch.schema === "vector180-patch/0.1" &&
        ...
        patch.ops.length === 3,
      "C10 patch envelope is not the expected minimal unified proposal",
    );
    assert(
    ...
    const patchCheck = runVector180Json(
    ...
    const patchApply = runVector180Json(
    ...
    const recoveredValidation = runVector180Json(
      ["validate", recoveredPath],
      stage,
    );
    assert(
    ...
    const regeneratedCompose = runVector180Json(
      [
        "compose",
        recoveredPath,
        "--placement",
    ...
    const regeneratedCompile = runVector180Json(
      [
        "compile",
        recoveredPath,
        "--placement",
    ...
    const regeneratedQuickLook = await captureQuickLook(
      ...
      "regenerated-from-recovered-vector180",
      stage,
    );
    assertSameCaptureProfile(
    ...
    const semanticComparison = {
      schema: "vector180-edited-regenerated-comparison/0.1",
      status: "passed",
      method: [
    ...
    const generationManifest = {
      schema: "office180-vector180-roundtrip-evidence-generation/0.1",
      generatedAt: new Date().toISOString(),
      generator: {
        path: "scripts/generate-vector180-roundtrip-evidence.mjs",
        sha256: await sha256File(
          join(
            REPO_ROOT,
            "scripts",
            "generate-vector180-roundtrip-evidence.mjs",
          ),
        ),
        runtime: {
        ...
        dependencies: [
          ...
          {
            path: "packages/vector180/src/node/pptx-baseline.ts",
            sha256: await sha256File(
              join(REPO_ROOT, "packages/vector180/src/node/pptx-baseline.ts"),
            ),
          },
          {
            path: "packages/vector180/src/node/reconcile.ts",
            sha256: await sha256File(
              join(REPO_ROOT, "packages/vector180/src/node/reconcile.ts"),
            ),
          },
        ],
      },
      source: {
        path: "supported-original.vector180.svg",
        ...
        font: {
          family: "ABeeZee",
          path: "packages/vector180/test-fixtures/fonts/ABeeZee-Regular.ttf",
          sha256: await sha256File(FONT_FILE),
          licensePath: "packages/vector180/test-fixtures/fonts/OFL.txt",
      ...
      commands: [
        [
          "node",
          "scripts/generate-vector180-roundtrip-evidence.mjs",
          "--destination",
          "tests/fixtures/roundtrip-evidence/vector180",
        ],
        [
          ".venv/bin/python",
