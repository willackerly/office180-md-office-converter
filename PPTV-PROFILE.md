# PPTV: Current PowerPoint Vector Profile

**Status:** 0.1 standalone diagram atom, HTML deck aggregation, C5 1.3 typed
...
atom placement/baseline, C10 1.2 proof-carrying reconciliation, and C11 1.1
...
PPTV is the current implementation name for a constrained, destination-neutral
...
PPTV compiler may represent it in PowerPoint.
...
single TypeScript package `@office180/pptv@0.1.0-alpha.4`. C5 1.3 preserves
...
plus one exact same-parent `pptv-patch/0.3` connector clone.
...
- C5 1.3 additionally supports old-value-preconditioned rect/ellipse geometry,
...
- C10 1.2 can inspect an authenticated edited descendant, prove named
...
old-value-preconditioned C5 1.3 permutation:
...
`src.*` names refuse by default. C10 1.2 has one narrow recovery path: exactly
...
`pptv-reconcile-resolution/0.1` document only when exactly one occurrence is
...
emits `pptv-patch/0.3` with exactly one `clone-connector`. Zero matches means
...
C5 1.3 tests cover every typed operation, mandatory old-value failures, unsafe
...
C4, C5 1.3, and C6 are the verified behavioral authorities for the implemented
...
native-calibration, quantitative comparison, human-review, representative
editing, or other promotion gates are open. This document remains the
author-facing rationale and broader roadmap.
...
`@office180/pptv@0.1.0-alpha.4`, C5 contract revision `1.3`, C9 revision
`1.0`, C10 revision `1.2`, C11 revision `1.1`, and the other current contract
revisions.
